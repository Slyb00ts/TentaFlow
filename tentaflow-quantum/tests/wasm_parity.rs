...
fn this_build_reproduces_the_parity_fixture() {
    ...
    for case in &fixture.cases {
        ...
        let result = run(&circuit, &options, Device::Cpu, case.shots, Cancel::none())
...
fn the_shot_stream_only_depends_on_the_seed() {
    ...
    let first = run(&circuit, &options, Device::Cpu, case.shots, Cancel::none()).expect("runs");
    let second = run(&circuit, &options, Device::Cpu, case.shots, Cancel::none()).expect("runs");
    ...
    let shifted = run(&circuit, &other, Device::Cpu, case.shots, Cancel::none()).expect("runs");
    assert_ne!(
