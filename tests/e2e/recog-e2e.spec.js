...
//   (typ recognition „Acme E2E", krok danych pominięty) → zakładka Dane:
...
const PROJECT_NAME = `Acme E2E ${Date.now()}`;
const DATASET_NAME = 'Acme mini';
...
(async () => {
  ...
  try {
    ...
    // ---- Krok 2: kreator 4-kroki → projekt recognition „Acme E2E" ----
    try {
      ...
      await page.locator('#ml-studio-wiz-desc textarea').first().fill('e2e recognition: pełny cykl RF-DETR (Acme)');
      ...
      if (outcome !== 'tabs' || !(await page.locator('#ml-studio-tabs').count())) {
        step('2. Kreator → projekt recognition „Acme E2E"', false,
      ...
      step('2. Kreator → projekt recognition „Acme E2E"', true,
