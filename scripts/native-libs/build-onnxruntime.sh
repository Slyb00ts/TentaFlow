...
#          linux-x86_64 and on Windows hosts with an NVIDIA GPU: the runtime
#          plus the CUDA/TensorRT provider libs, so the `ort` crate with
#          load-dynamic can register TensorRT/CUDA EPs). `--from-source` builds
#          from the pinned tag with native SM cubins.
#
# Every version and checksum used here (ONNX Runtime archives, the aarch64 GPU
# wheel, TensorRT/cuDNN/CUDA wheels) comes from scripts/versions.env.
#
...
#     TensorRT EP loads nvinfer 10 at runtime.
...
#   the TensorRT, cuDNN and CUDA toolkit runtime libs from the official NVIDIA
#   wheels (pypi.nvidia.com, no login) into lib-dynamic/, flat, so
#   tentaflow/build.rs copies them next to the binary. Linux resolves them via
#   rpath/$ORIGIN, Windows through the executable's directory (first in the DLL
#   search order). Vendored set:
#     TensorRT: nvinfer 10, nvinfer_plugin 10, nvonnxparser 10 and the builder
#               resources (per-SM since TRT 10.15; the ptx one always)
#     cuDNN 9:  cudnn + every split lib it loads (ops/cnn/adv/graph/heuristic/
#               engines_*)
#     CUDA toolkit runtime (the EPs need these and a driver-only host has none
#     of them): cudart 13, cublas + cublasLt 13, cufft 12, curand 10.
#   This is ~3 GB on disk (and again next to the binary). Opt-outs:
...
# NVIDIA wheels: platform tag in the wheel filename and the key suffix of their
# checksums in versions.env.
case "$PLATFORM" in
  linux-x86_64)   WHEEL_TAG="x86_64";    NVIDIA_PLATFORM="LINUX_X86_64" ;;
  linux-aarch64)  WHEEL_TAG="aarch64";   NVIDIA_PLATFORM="LINUX_AARCH64" ;;
  windows-x86_64) WHEEL_TAG="win_amd64"; NVIDIA_PLATFORM="WINDOWS_X86_64" ;;
  *)              WHEEL_TAG="";          NVIDIA_PLATFORM="" ;;
esac

# Fails unless the file's SHA-256 equals the expected value.
verify_checksum() {
  local path="$1" expected="$2" actual
  actual="$(sha256_of "$path")"
  if [ "$actual" != "$expected" ]; then
    echo "ERROR: SHA-256 mismatch for $(basename "$path")" >&2
    echo "  expected: $expected (scripts/versions.env)" >&2
    echo "  actual:   $actual" >&2
    rm -f "$path"
    echo "Removed $path (corrupted or tampered download); run again to re-download." >&2
    return 1
  fi
  echo ">>> SHA-256 verified: $(basename "$path")"
}

# onnxruntime-<os-arch-variant>-<version>.<ext> -> ONNXRUNTIME_SHA256_<OS_ARCH_VARIANT>
archive_checksum() {
  local archive="$1" stem
  stem="${archive#onnxruntime-}"
  stem="${stem%-"${ONNXRUNTIME_REF#v}".tgz}"
  stem="${stem%-"${ONNXRUNTIME_REF#v}".zip}"
  pinned_checksum "ONNXRUNTIME_SHA256_$(printf '%s' "$stem" | tr 'a-z-' 'A-Z_')" ONNXRUNTIME_REF
}
...
# index — the manylinux platform tag varies per package/version (e.g. TRT is
# manylinux_2_28_x86_64 vs manylinux_2_35_aarch64), so it cannot be
# reconstructed from the version alone.
resolve_nvidia_wheel() {
  ...
  filename="$(curl -fsSL --max-time 30 "https://pypi.nvidia.com/$package/" \
    | grep -oE "href=\"${underscored}-${version}-[^\"#]*${WHEEL_TAG}[^\"#]*\\.whl" \
    | head -n1 | sed 's/^href="//')"
  if [ -z "$filename" ]; then
    echo "ERROR: no $WHEEL_TAG wheel for $package==$version on pypi.nvidia.com" >&2
    return 1
  fi
  printf '%s\n' "$filename"
}

# Downloads and verifies one NVIDIA wheel; prints its local path.
# $2 names the version variable, $3 the component in its checksum key
# (NVIDIA_WHEEL_SHA256_<component>_<platform>).
fetch_nvidia_wheel() {
  local package="$1" version_key="$2" component="$3"
  local version wheel path expected
  version="$(require_version "$version_key")"
  expected="$(pinned_checksum "NVIDIA_WHEEL_SHA256_${component}_${NVIDIA_PLATFORM}" "$version_key")" || return 1
  wheel="$(resolve_nvidia_wheel "$package" "$version")" || return 1
  path="$NATIVE_CACHE/downloads/$wheel"
  download_cached "https://pypi.nvidia.com/$package/$wheel" "$path" >&2 || return 1
  verify_checksum "$path" "$expected" >&2 || return 1
  printf '%s\n' "$path"
}

# Extracts wheel members matching a python regex flat (basename only) into a
# directory. Wheels are plain zips; Python is already a script prerequisite.
extract_wheel_members() {
  local wheel_path="$1"
  local pattern="$2"
  local dest="$3"
  WHEEL="$wheel_path" PATTERN="$pattern" DEST="$dest" "$(python_cmd)" - <<'PYEOF'
...
# Python's zipfile needs a native path on Windows.
native_path() {
  if is_windows_host; then cygpath -w "$1"; else printf '%s\n' "$1"; fi
}
...
detect_trt_sm_bucket() {
  local cap
  cap="$(nvidia-smi --query-gpu=compute_cap --format=csv,noheader 2>/dev/null | head -n1 | tr -d ' \r' || true)"
  case "$cap" in
    7.5) echo "sm75" ;;
    8.0|8.7) echo "sm80" ;;
    8.6) echo "sm86" ;;
...
# Library file patterns per platform. Linux wheels keep shared objects under
# nvidia/<pkg>/lib, Windows wheels keep DLLs under nvidia/<pkg>/bin.
if is_windows_host; then
  CUDA_MEMBER_DIR='nvidia/cu13/bin/x86_64'
  CUDNN_MEMBER='^nvidia/cudnn/bin/cudnn[a-z_]*64_9\.dll$'
  CUDNN_GLOB='cudnn*64_9.dll'
  TRT_GLOBS='nvinfer*.dll nvonnxparser*.dll'
  CUDA_GLOBS='cudart64_*.dll cublas64_*.dll cublasLt64_*.dll cufft64_*.dll curand64_*.dll'
else
  CUDA_MEMBER_DIR='nvidia/cu13/lib'
  CUDNN_MEMBER='^nvidia/cudnn/lib/libcudnn.*\.so\.9.*$'
  CUDNN_GLOB='libcudnn*.so*'
  TRT_GLOBS='libnvinfer*.so* libnvonnxparser*.so*'
  CUDA_GLOBS='libcudart*.so* libcublas*.so* libcufft*.so* libcurand*.so*'
fi

# Removes previously vendored files matching space-separated globs.
remove_vendored() {
  local lib_dir="$1" glob
  for glob in $2; do
    rm -f "$lib_dir"/$glob 2>/dev/null || true
  done
}
...
vendor_nvidia_runtimes() {
  ...
  local native_lib_dir wheel
  native_lib_dir="$(native_path "$lib_dir")"
  mkdir -p "$NATIVE_CACHE/downloads"
  local required_libs=()

  # --- TensorRT runtime. Skipped (CUDA EP only) when
  # TENTAFLOW_SKIP_TENSORRT_VENDOR=1 — default on aarch64/GB10 where sm_121
  # TRT is unproven; ort_common falls TensorRT->CUDA->CPU softly. ---
  remove_vendored "$lib_dir" "$TRT_GLOBS"
  if [ "${TENTAFLOW_SKIP_TENSORRT_VENDOR:-0}" = "1" ]; then
    echo ">>> Skipping TensorRT vendoring (TENTAFLOW_SKIP_TENSORRT_VENDOR=1 — CUDA EP only)."
  else
    wheel="$(fetch_nvidia_wheel tensorrt-cu13-libs TENSORRT_VENDOR_REF TENSORRT)" || return 1
    ...
    # The optional suffix group matches only ptx + the selected SM buckets;
    # the empty-suffix branch keeps a monolithic builder resource working.
    local trt_pattern
    if is_windows_host; then
      trt_pattern="^tensorrt_libs/(nvinfer_10\\.dll|nvinfer_plugin_10\\.dll|nvonnxparser_10\\.dll|nvinfer_builder_resource(_(ptx${bucket_re}))?_10\\.dll)$"
      required_libs+=(nvinfer_10.dll nvonnxparser_10.dll)
    else
      trt_pattern="^tensorrt_libs/(libnvinfer\\.so\\.10|libnvinfer_plugin\\.so\\.10|libnvonnxparser\\.so\\.10|libnvinfer_builder_resource(_(ptx${bucket_re}))?\\.so\\..*)$"
    ...
    extract_wheel_members "$(native_path "$wheel")" "$trt_pattern" "$native_lib_dir" || return 1
    # A builder resource MUST have landed: either a monolithic file or at least
    # one per-SM/ptx variant. A typo in TENSORRT_SMS would otherwise ship a
    # runtime that cannot build engines.
    if ! ls "$lib_dir"/*nvinfer_builder_resource* >/dev/null 2>&1; then
      echo "ERROR: no nvinfer_builder_resource* extracted — check TENSORRT_SMS ('$sms') against the wheel contents." >&2
      return 1
    fi
  fi

  # --- cuDNN 9: the loader lib dlopens the split libs, so the whole set. ---
  wheel="$(fetch_nvidia_wheel nvidia-cudnn-cu13 CUDNN_VENDOR_REF CUDNN)" || return 1
  remove_vendored "$lib_dir" "$CUDNN_GLOB"
  echo ">>> Vendoring cuDNN $CUDNN_VENDOR_REF runtime:"
  extract_wheel_members "$(native_path "$wheel")" "$CUDNN_MEMBER" "$native_lib_dir" || return 1
  if is_windows_host; then required_libs+=(cudnn64_9.dll); else required_libs+=(libcudnn.so.9); fi

  # --- CUDA toolkit runtime libs the EPs need (a driver-only host has no
  # toolkit): cudart, cublas/cublasLt, cufft, curand. nvblas and cufftw are
  # deliberately excluded — nothing in the EP chain references them. ---
  if [ "${TENTAFLOW_SKIP_CUDA_VENDOR:-0}" = "1" ]; then
    ...
    remove_vendored "$lib_dir" "$CUDA_GLOBS"
    local component package version_key linux_re windows_re
    while read -r component package version_key linux_re windows_re; do
      wheel="$(fetch_nvidia_wheel "$package" "$version_key" "$component")" || return 1
      echo ">>> Vendoring $package ${!version_key}:"
      if is_windows_host; then
        extract_wheel_members "$(native_path "$wheel")" "^$CUDA_MEMBER_DIR/($windows_re)\$" "$native_lib_dir" || return 1
      else
        extract_wheel_members "$wheel" "^$CUDA_MEMBER_DIR/($linux_re)\$" "$lib_dir" || return 1
      fi
    done <<'EOF'
CUDART nvidia-cuda-runtime CUDART_VENDOR_REF libcudart\.so\..* cudart64_13\.dll
CUBLAS nvidia-cublas CUBLAS_VENDOR_REF libcublas(Lt)?\.so\..* cublas(Lt)?64_13\.dll
CUFFT nvidia-cufft CUFFT_VENDOR_REF libcufft\.so\..* cufft64_12\.dll
CURAND nvidia-curand CURAND_VENDOR_REF libcurand\.so\..* curand64_10\.dll
EOF
    if is_windows_host; then
      required_libs+=(cudart64_13.dll cublas64_13.dll cublasLt64_13.dll cufft64_12.dll curand64_10.dll)
    else
      required_libs+=(libcudart.so.13 libcublas.so.13 libcublasLt.so.13 libcufft.so.12 libcurand.so.10)
    fi
  fi

  # --- Post-vendor sanity: every library the EPs load must be in the
  # vendored dir (exactly what $ORIGIN / the executable directory gives the
  # binary). ---
  local missing=0 lib
  ...
  if ! is_windows_host && command -v ldd >/dev/null 2>&1; then
  ...
  vendored_size="$(cd "$lib_dir" && du -shc $TRT_GLOBS $CUDNN_GLOB $CUDA_GLOBS 2>/dev/null | tail -n1 | awk '{print $1}' || true)"
...
# so a plain build-all run auto-provisions the GPU stack only where it helps).
gpu_present() {
...
# driver-supported CUDA version from nvidia-smi (both the classic "CUDA
# Version:" and the newer "CUDA UMD Version:" header formats); drivers
...
# Windows counterpart: the GPU archive must have delivered the runtime and the
# three provider DLLs the CUDA/TensorRT EPs are loaded from.
sanity_check_gpu_windows() {
  local lib_dir="$NATIVE_ROOT/$PLATFORM/lib-dynamic" dll missing=0
  for dll in onnxruntime.dll onnxruntime_providers_shared.dll onnxruntime_providers_cuda.dll onnxruntime_providers_tensorrt.dll; do
    if [ ! -f "$lib_dir/$dll" ]; then
      echo "ERROR: expected $dll missing in $lib_dir" >&2
      missing=1
    fi
  done
  [ "$missing" -eq 0 ] || return 1
  echo ">>> ONNX Runtime GPU providers: $(cd "$lib_dir" && ls onnxruntime_providers_*.dll | tr '\n' ' ')"
}

# Removes stale runtime versions before copying the new one — otherwise after a
# version bump lib-dynamic holds two files (e.g. libonnxruntime.so.1.26.0 +
# .1.30.0) and the ort load-dynamic probe picks non-deterministically. A CPU
# archive after a GPU one must not leave orphaned provider DLLs either.
clean_stale_runtime() {
...
# x64 GPU archives ship — next to the runtime — the providers_{shared,cuda,
# tensorrt} libs, so the `ort` crate (load-dynamic, ORT_DYLIB_PATH -> the
# runtime) can register the TensorRT/CUDA EPs. A system libonnxruntime usually
# has CUDA at best (or nothing).
# CUDA variant: `gpu_cuda12` = CUDA 12 / cuDNN 9, `gpu_cuda13` = CUDA 13 /
# cuDNN 9 / TensorRT >= 10.13 EP — the latter is required for SM_103 (B300);
# kernels ship as PTX so they also JIT on future SMs. When the runtime cannot
# load the TRT provider, detector code falls back to the CUDA EP gracefully.
# Force CPU-only with ONNXRUNTIME_GPU=0.
# macos-arm64: the osx-arm64 archive has the CoreML EP built in (Metal/ANE) —
# no GPU variant exists or is needed.
case "$PLATFORM" in
  windows-*)
    if [ -z "${ONNXRUNTIME_GPU:-}" ]; then
      if gpu_present; then ONNXRUNTIME_GPU=1; else ONNXRUNTIME_GPU=0; fi
    fi
    ;;
...
case "$PLATFORM" in
  linux-x86_64|windows-x86_64)
    case "$PLATFORM" in
      linux-*) ARCHIVE_OS="linux-x64"; ARCHIVE_EXT="tgz" ;;
      *)       ARCHIVE_OS="win-x64";   ARCHIVE_EXT="zip" ;;
    esac
    if [ "$ONNXRUNTIME_GPU" = "1" ]; then
      CUDA_MAJOR="$(detect_cuda_major)"
      case "$CUDA_MAJOR" in
        12|13) ARCHIVE="onnxruntime-$ARCHIVE_OS-gpu_cuda$CUDA_MAJOR-$VERSION.$ARCHIVE_EXT" ;;
      ...
      ARCHIVE="onnxruntime-$ARCHIVE_OS-$VERSION.$ARCHIVE_EXT"
    fi
    ;;
...
ARCHIVE_SHA256="$(archive_checksum "$ARCHIVE")"

download_cached "$URL" "$ARCHIVE_PATH"
...
if [ "$GPU_ARCHIVE" = "1" ] && [ "$CUDA_MAJOR" = "13" ]; then
  vendor_nvidia_runtimes
fi
