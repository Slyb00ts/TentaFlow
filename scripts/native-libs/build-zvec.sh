...
case "$PLATFORM" in
  linux-*|macos-*)
    ...
    append_manifest_library "$PLATFORM" "zvec" "dynamic" "$ZVEC_REF" "Desktopowy build upstream produkuje samowystarczalną bibliotekę współdzieloną."
    ;;
  windows-*)
    copy_matching "$SRC_DIR" "$STATIC_DIR" -name 'zvec_c_api.lib'
    copy_matching "$SRC_DIR" "$DYNAMIC_DIR" -name 'zvec_c_api.dll'
    append_manifest_library "$PLATFORM" "zvec" "dynamic-import-lib" "$ZVEC_REF" "MSVC używa import library, a DLL musi być obok binarki."
    ;;
  ios-*|android-*)
    copy_matching "$SRC_DIR" "$STATIC_DIR" -name 'libzvec_c_api.a' -o -name 'libzvec_deps.a'
    append_manifest_library "$PLATFORM" "zvec" "static" "$ZVEC_REF" "Mobile używa archiwów statycznych."
    ;;
  *)
