...
/// `mel_filters` with norm="slaney", mel_scale="slaney").
...
mod tests {
    ...
    fn silence_produces_flat_floor() {
        let spec = log_mel_spectrogram(&vec![0.0; SAMPLE_RATE]);
    ...
    fn tone_is_deterministic_and_localized() {
        ...
        let tone: Vec<f32> = (0..SAMPLE_RATE)
            ...
            .collect();
        let a = log_mel_spectrogram(&tone);
        let b = log_mel_spectrogram(&tone);
        assert_eq!(a, b);
