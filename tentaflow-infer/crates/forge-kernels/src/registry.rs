...
/// Artefakty AMD są code objectami (HSACO), nie tekstem PTX, i mieszkają w
/// katalogu swojej architektury. Poza rozszerzeniem i ścieżką kontrakt jest ten
/// sam, co dla NVIDII: lista nazw musi zgadzać się z manifestem, a rozjazd
/// wychodzi przy ładowaniu, nie przy pierwszym uruchomieniu kernela.
macro_rules! embedded_gfx1030 {
    ($($name:literal),+ $(,)?) => {
        &[$(EmbeddedArtifact {
            name: $name,
            ptx: include_bytes!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../../kernels/mojo/build/gfx1030/",
                $name,
                ".hsaco"
            )),
        }),+]
