...
use forge_formats::source::{GgufSource, HostWeight, StSource, TensorSource};
...
impl ModelWeights {
    ...
    pub fn load_safetensors_dir(
        ...
        // dalej idzie tą samą ścieżką co każdy inny (`forge_formats::mlx_source`).
        let mlx_src = forge_formats::mlx_source::MlxSource::detect(&config_text, &st);
        let st_src = StSource {
            st: &st,
            scheme: scheme.clone(),
            fp8,
