...
use forge_formats::{
    dequantize_to_f32, repack_affine_to_q4_1, BlockMatrix, Gguf, HfConfig, LayerKind,
    MlxAffineTensor, MlxParams, MlxQuantConfig, ModelDescriptor, RoleShard, TpShard, WeightRole,
};
...
impl ModelWeights {
    ...
    pub fn load_safetensors_dir(
        ...
        // Checkpoint MLX rozpoznaje się po bloku kwantyzacji w config.json.
        // Dalej jest tą samą ścieżką co każdy inny: wagi wchodzą przez
        // `TensorSource`, a MLX affine 4-bit przekłada się na Q4_1, który
        // silnik już liczy — bez żadnego nowego kernela.
        let mlx = serde_json::from_str::<serde_json::Value>(&config_text)
            .ok()
        ...
        let mlx_src = mlx.map(|cfg| MlxSource::new(&st, cfg));
        let src: &dyn TensorSource = match &mlx_src {
            Some(s) => s,
            None => &st_src,
        };
