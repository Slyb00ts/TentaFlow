# =============================================================================
# Plik: nvfp4_tile128_mma.mojo
# Opis: Dokladny pipeline NVFP4 FP16 MMA dla ukladu tile-major N128/K64.
# Przyklad: gemm_nvfp4_tile128_mma_f16_bm128_bn128 liczy prefill.
# =============================================================================
...
def gemm_nvfp4_tile128_mma_sync1_impl[BN: Int, N_TILES: Int](
    ...
    if source_row > n_rows - 1:
        source_row = n_rows - 1
    weight_tile = source_row // 128
    weight_layout_thread = (source_row % 128) * 2 + weight_subblock
    weight_base = weights + weight_tile * blocks_per_row * 128 * 36
    ...
    def fetch_raw(
        ...
        weight_layout_thread: Int,
        mut raw_codes: SIMD[DType.uint8, 8],
        mut raw_scale: UInt8,
    ):
        block = stage // 2
        stage_base = (
            block * 128 * 36 + (stage % 2) * 128 * 18
        )
        raw_codes = (weight_base + stage_base + 128 * 2
            + weight_layout_thread * 8).load[
            width=8, alignment=8
        ]()
        raw_scale = weight_base[stage_base + weight_layout_thread]
    ...
    if weight_row < BN:
        fetch_raw(
            0, weight_subblock, weight_base, weight_layout_thread,
            raw_codes, raw_scale,
        )
...
comptime gemm_nvfp4_tile128_mma_f16_bm128_bn64 = (
    gemm_nvfp4_tile128_mma_sync1_impl[64, 4]
)
comptime gemm_nvfp4_tile128_mma_f16_bm128_bn128 = (
    gemm_nvfp4_tile128_mma_sync1_impl[128, 8]
)
