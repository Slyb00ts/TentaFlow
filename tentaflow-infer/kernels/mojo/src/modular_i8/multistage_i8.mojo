...
@always_inline
def _accum_type[t: DType]() -> DType:
    # get_accum_type[int8] returns int8; the s8 IMMA accumulates in int32.
    comptime if t == DType.int8:
        return DType.int32
    else:
        return get_accum_type[t]()


from linalg.utils import apply_epilogue, elementwise_epilogue_type
from linalg.utils_gpu import MatmulConfig, block_swizzle
from linalg.matmul.gpu.amd import AMDMatmul
...
def multistage_mma[
    ...
    *,
    ...
    comptime mma_shape = get_mma_shape[a_type, _accum_type[a_type]()]()
    ...
    comptime accum_type = _accum_type[a_type]()
...
def multistage_gemm_kernel[
    ...
    ), "Pipeline gemm only supports tf32, F16, BF16, E4M3, E5M2, and S8 mma"
