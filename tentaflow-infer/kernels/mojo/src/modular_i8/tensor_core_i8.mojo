...
# int8 tensor-core mma (m16n8k32 s8×s8→s32) — the stdlib `mma()` free function
# has NO NVIDIA int8 arm, so the s8 IMMA is emitted via raw inline asm exactly
# like FORGE's `_mma_s8`. Fragment register layout is byte-identical to fp8:
# A = 16×int8 (4 u32 regs), B = 8×int8 (2 u32 regs), C = 4×int32.
@always_inline
def _mma_s8_frag(
    ...
    var au = bitcast[DType.uint32, 4](a)
    var bu = bitcast[DType.uint32, 2](b)
    var r = inlined_assembly[
        ...
        au[0], au[1], au[2], au[3], bu[0], bu[1], c[0], c[1], c[2], c[3]
    )
...
struct TensorCore[
    ...
    comptime supported_int8 = (
        Self.in_type == DType.int8
        and Self.out_type == DType.int32
        and Self.shape == shape_16x8x32
    ) if is_nvidia_gpu() else False
    """Whether int8 (s8×s8→s32, m16n8k32) is supported (NVIDIA sm_80+)."""
    ...
    def _load_b_nvidia(
        ...
        comptime swizzle = Swizzle(0, 0, 1) if not Self.b_swizzle else make_ldmatrix_swizzle[
        ...
        comptime if Self.transpose_b:
            comptime if Self.in_type == DType.float32:
                ...
                comptime assert self.supported_half or self.supported_fp8 or self.supported_int8, (
                    "Transposed matrix B is only supported for half, fp8 and int8"
                    " data types."
                )
    ...
    def mma(
        ...
        comptime for m_mma in range(num_m_mmas):
            comptime for n_mma in range(num_n_mmas):
                ...
                comptime if Self.supported_int8:
                    c_frag[idx, 0] = rebind[type_of(c_frag[idx, 0])](
                        _mma_s8_frag(
                            rebind[SIMD[DType.int8, 16]](a_frag[m_mma, 0]),
                            rebind[SIMD[DType.int8, 8]](b_frag[n_mma, 0]),
                            rebind[SIMD[DType.int32, 4]](c_frag[idx, 0]),
                        )
                    )
                else:
                    mma(
...
def get_mma_shape[
    ...
    comptime if has_nvidia_gpu_accelerator():
        comptime if accum_type == DType.float32 and input_type == DType.float32:
            ...
            return shape_16x8x32
        elif accum_type == DType.int32 and input_type == DType.int8:
            return shape_16x8x32
        else:
