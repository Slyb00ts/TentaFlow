# Golden test kafla Q2_K WMMA: wynik GPU wobec referencji liczonej na hoscie
# dokladnie ta sama formula co `gemm_q5_k_impl` (q5 * d*sc - dmin*mn).
from std.gpu.host import DeviceContext
from std.random import random_si64, seed

from src.gemm_q2_k_wmma import gemm_q2_k_wmma_f16_bm32, gemm_q2_k_wmma_f16_bm256
...
def reference(w: UnsafePointer[UInt8, MutUntrackedOrigin], row: Int, col: Int, n_cols: Int) -> Float32:
    ...
    n = r // 128
    sh = (r % 128) // 32
    scb = (w + sb + n * 8 + 2 * sh + (r % 32) // 16)[0]
    d = Float32((w + sb + 80).bitcast[Float16]().load[width=1, alignment=1]()[0])
    dmin = Float32((w + sb + 82).bitcast[Float16]().load[width=1, alignment=1]()[0])
    q = Int(((w + sb + 16 + n * 32 + (r % 32))[0] >> UInt8(2 * sh)) & 3)
    return Float32(q) * (d * Float32(Int(scb & 0x0F))) - dmin * Float32(Int(scb >> 4))
...
def run(ctx: DeviceContext, n_rows: Int, n_cols: Int, n_tokens: Int, big: Bool) raises:
    ...
    for r in range(n_rows):
        for s in range(sbs):
            ...
            wh[base + 80] = 0x00
            wh[base + 81] = 0x2C     # d ~0.0625
            wh[base + 82] = 0x00
            wh[base + 83] = 0x28     # dmin ~0.03125
    ...
    if big:
        comptime BM = 256
        comptime BN = 64
        ctx.enqueue_function[gemm_q2_k_wmma_f16_bm256](
        ...
        comptime BM = 32
        comptime BN = 64
        ctx.enqueue_function[gemm_q2_k_wmma_f16_bm32](
            y.unsafe_ptr(), w.unsafe_ptr(), x.unsafe_ptr(),
            n_cols, n_rows, n_tokens,
            grid_dim=((n_rows + BN - 1) // BN, (n_tokens + BM - 1) // BM),
            block_dim=128,
