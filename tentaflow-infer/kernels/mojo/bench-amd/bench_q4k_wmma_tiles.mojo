...
def one[WM: Int, WN: Int, MT: Int, NT: Int](
    ...
    for i in range(WARMUP + ITERS):
        if i == WARMUP:
            ctx.synchronize()
            t0 = perf_counter_ns()
        ctx.enqueue_function[gemm_q4_k_wmma_impl[WM, WN, MT, NT]](
...
def shape(ctx: DeviceContext, n_rows: Int, n_cols: Int, n_tokens: Int) raises:
    ...
    one[4, 2, 4, 2](ctx, String("obecny BM256/BN64"), yp, wp, xp, n_rows, n_cols, n_tokens, flops)
    one[2, 2, 4, 2](ctx, String("BM128/BN64      "), yp, wp, xp, n_rows, n_cols, n_tokens, flops)
    one[2, 2, 4, 4](ctx, String("BM128/BN128     "), yp, wp, xp, n_rows, n_cols, n_tokens, flops)
    one[2, 4, 4, 2](ctx, String("BM128/BN128 8fal"), yp, wp, xp, n_rows, n_cols, n_tokens, flops)
    one[4, 2, 2, 2](ctx, String("BM128/BN64 8fal "), yp, wp, xp, n_rows, n_cols, n_tokens, flops)
    one[2, 2, 2, 4](ctx, String("BM64/BN128      "), yp, wp, xp, n_rows, n_cols, n_tokens, flops)
