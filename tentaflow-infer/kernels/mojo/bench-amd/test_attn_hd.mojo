...
def check[head_dim: Int, PT: Int](
    ctx: DeviceContext, n_tokens: Int, n_q_heads: Int, n_kv_heads: Int
) raises:
    comptime PAGE = 32
    ...
    ctx.enqueue_function[attn_prefill[head_dim, DType.float16, PT]](
        ...
        pd.unsafe_ptr(), 0, n_q_heads, n_kv_heads, PAGE, scale, n_tokens,
        ...
        for t in range(n_tokens):
            for h in range(n_q_heads):
                ...
                for e in range(head_dim):
                    ...
                    if d > worst:
                        worst = d
    print("hd", head_dim, "PT", PT, "T", n_tokens, "blad max:", worst)
    if worst > 0.004:
...
def main() raises:
    ...
    check[256, WARP](ctx, 20, 4, 2)
    check[512, WARP // 2](ctx, 20, 4, 1)
    check[512, WARP // 2](ctx, 40, 2, 1)
