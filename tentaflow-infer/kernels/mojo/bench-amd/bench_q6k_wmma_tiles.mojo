# =============================================================================
# Plik: bench_q6k_wmma_tiles.mojo
# Opis: Sweep kafli prefillowego GEMM-u Q6_K na WMMA, na ksztaltach Q6_K
#       ThinkingCap-Qwen3.6-27B (ffn_down i attn_qkv). Dobor kafla ma wynikac
#       z pomiaru, a nie z rachunku.
# Przyklad: pixi run mojo run -I . bench-amd/bench_q6k_wmma_tiles.mojo
# =============================================================================
from std.gpu.host import DeviceContext
from std.random import random_si64, seed
from std.time import perf_counter_ns

from src.gemm_q6_k_wmma import gemm_q6_k_wmma_impl
...
def one[WM: Int, WN: Int, MT: Int, NT: Int, PAD: Int = 0](
    ...
    for i in range(WARMUP + ITERS):
        if i == WARMUP:
            ctx.synchronize()
            t0 = perf_counter_ns()
        ctx.enqueue_function[gemm_q6_k_wmma_impl[WM, WN, MT, NT, PAD]](
            y,
            w,
            x,
            n_cols,
...
def main() raises:
    ...
    shape(ctx, 5120, 17408, 512)
    # attn_qkv 27B
    shape(ctx, 10240, 5120, 1024)
