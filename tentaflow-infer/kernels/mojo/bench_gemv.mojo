...
def main() raises:
    ...
    # q4_k v2 (300-launch warmup so the GPU reaches boost clocks)
    var wk4 = ctx.enqueue_create_buffer[DType.uint8](ROWSQ * (COLS // 256) * 144)
    for _ in range(300):
        ctx.enqueue_function[gemv_q4_k_f16_v2](y.unsafe_ptr(), wk4.unsafe_ptr(), x.unsafe_ptr(), COLS, ROWSQ, grid_dim=(ROWSQ + 7) // 8, block_dim=256)
    ctx.synchronize()
    t0 = perf_counter_ns()
    for _ in range(ITERS):
        ctx.enqueue_function[gemv_q4_k_f16_v2](y.unsafe_ptr(), wk4.unsafe_ptr(), x.unsafe_ptr(), COLS, ROWSQ, grid_dim=(ROWSQ + 7) // 8, block_dim=256)
