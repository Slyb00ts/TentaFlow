# Golden test kafla Q5_K WMMA: wynik GPU wobec referencji liczonej na hoscie
# dokladnie ta sama formula co `gemm_q5_k_impl` (q5 * d*sc - dmin*mn).
from std.gpu.host import DeviceContext
from std.random import random_si64, seed

from src.gemm_q5_k_wmma import gemm_q5_k_wmma_f16_bm32, gemm_q5_k_wmma_f16_bm256
...
def run(ctx: DeviceContext, n_rows: Int, n_cols: Int, n_tokens: Int, big: Bool) raises:
    ...
    if big:
        comptime BM = 256
        comptime BN = 64
        ctx.enqueue_function[gemm_q5_k_wmma_f16_bm256](
        ...
        comptime BM = 32
        comptime BN = 64
        ctx.enqueue_function[gemm_q5_k_wmma_f16_bm32](
            y.unsafe_ptr(), w.unsafe_ptr(), x.unsafe_ptr(),
            n_cols, n_rows, n_tokens,
            grid_dim=((n_rows + BN - 1) // BN, (n_tokens + BM - 1) // BM),
            block_dim=128,
