...
// docs/BENCH_COMPARISON.md W4A8 section). Reaches ~450 TFLOP-eq on Mistral FFN
// shapes = 2.2x llama.cpp's MMQ (206) on the RTX 4090's int8 tensor cores — the
// only Marlin-family variant that uses the int8 (not fp16) TC path.
//
// I/O CONTRACT (QServe layout — validated byte-exact vs a CPU int4xint8 golden in
//   scratch/w4a8/harness.cu, relL2 ~2e-4 = fp16 noise):
//   A     : i8   [M=tokens][K]                 per-token int8 activation codes
//   B     : i8   [N/32][K/32][32][16] (packed)  int4 weight, 8-D interleave
//   zeros : i8   [K/G][N] (reordered)          (-zero)*s2 per group, 2's complement
//   s2    : i8   [K/G][N] (reordered)          per-group int8 secondary scale
//   ws    : f16  [N]                           per-channel primary scale (s1)
//   as    : f16  [M]                           per-token activation scale
//   C     : f16  [M][N]                        C[m][n] = as[m]*s1[n]*Sum_k a*s2*(q4-zero)
// The weight/scale/zero packing that produces these buffers from an fp16/Q4_K
// tensor is reproduced in forge-formats (w4a8 requant); the per-token int8 quant
// is `quantize_act_w4a8` (kernels/mojo). group_size G is a compile-time 128.
//
// dense_kernel0 + its device helpers below are QServe's, unmodified. Only the
// extern-C entry points (one per CTA config) and the host grid math (in the Rust
// launcher) are FORGE's — a cubin loaded through the same cuModuleLoadData path.
...
template <int CTA_M, int CTA_N, int CTA_K, int WARP_M, int WARP_N, int WARP_K,
          int STAGES, int G>
__device__ void dense_kernel0(int8_t *__restrict__ A, int8_t *__restrict__ B,
...
// ---- Per-token symmetric int8 activation quantizer for the W4A8 GEMM. Produces
    ...
    half* __restrict__ ascales, const half* __restrict__ inv_smooth, int M, int64_t K)
{
  ...
  if (wid == 0) {
    amax = (lane < nwarps) ? red[lane] : 0.f;
    for (int o = 16; o > 0; o >>= 1)
      amax = fmaxf(amax, __shfl_down_sync(0xffffffff, amax, o));
    if (lane == 0) red[0] = amax;
  }
  __syncthreads();
  // fp16-round the scale so the codes match the QServe packer's fp16 primary
  // scale convention (the CPU golden uses an f16 activation scale too).
  float s = red[0] > 0.f ? red[0] / 127.f : 1.f;
  half sh = __float2half(s);
...
// ---- FORGE extern-C entry points (one per QServe CTA config). The Rust launcher
// (forge-kernels) computes grid/block/dynamic-smem exactly as QServe's host
// KERNEL_LAUNCH_CODE. All configs use <= 48 KB smem so no cudaFuncSetAttribute
// is needed. G is fixed at 128 (the compile-time group size). ----
#define W4A8_ENTRY(NAME, CTA_M, CTA_N, CTA_K, WARP_M, WARP_N, WARP_K, STAGES)      \
    extern "C" __global__ void __launch_bounds__(                                  \
        ((CTA_M) / (WARP_M)) * ((CTA_N) / (WARP_N)) * ((CTA_K) / (WARP_K)) * 32)   \
    NAME(int8_t* A, int8_t* B, int8_t* zeros, int8_t* s2, half2* wscales,          \
         half* ascales, half* C, int M, int64_t N, int64_t K) {                    \
        dense_kernel0<CTA_M, CTA_N, CTA_K, WARP_M, WARP_N, WARP_K, STAGES, 128>(   \
            A, B, zeros, s2, wscales, ascales, C, M, N, K);                        \
    }

W4A8_ENTRY(forge_w4a8_gemm_m128,    128, 64,  64, 64, 32, 64, 4)
W4A8_ENTRY(forge_w4a8_gemm_m64_ksm,  64, 64,  64, 32, 32, 64, 4)
W4A8_ENTRY(forge_w4a8_gemm_m64_klg,  64, 64, 128, 32, 32, 64, 3)
W4A8_ENTRY(forge_w4a8_gemm_m32,      32, 64, 128, 32, 32, 64, 3)
