...
mod tests {
    ...
    fn button_roundtrip() {
        let v = Button {
            variant: ButtonVariant::Primary, tone: Tone::Critical,
            label: lit("Save"),
            icon_leading: Some(icon()), icon_trailing: None,
            size: ButtonSize::Lg, full_width: true,
            disabled: None, loading: None, density: Density::Compact,
        };
        rt(v, |m| m.into_component("b").unwrap(), Button::try_from_component);
    }

    #[test]
    fn icon_button_roundtrip() {
        let v = IconButton {
            icon: icon(), variant: ButtonVariant::Ghost, tone: Tone::Neutral,
            size: ButtonSize::Sm, aria_label: "Close".into(),
            disabled: None, loading: None,
        };
        rt(v, |m| m.into_component("ib").unwrap(), IconButton::try_from_component);
    }
    ...
    fn button_group_roundtrip() {
        let v = ButtonGroup {
            ...
            orientation: ButtonGroupOrientation::Horizontal, attached: true,
        };
        rt(v, |m| m.into_component("bg").unwrap(), ButtonGroup::try_from_component);
    }

    #[test]
    fn link_button_roundtrip() {
        let v = LinkButton {
            label: lit("Open"), icon_leading: None, icon_trailing: None,
            tone: Tone::Primary, underline: LinkUnderline::Hover,
        };
        rt(v, |m| m.into_component("lb").unwrap(), LinkButton::try_from_component);
    }

    #[test]
    fn link_roundtrip() {
        let v = Link {
            label: lit("Read more"), underline: LinkUnderline::Always,
            tone: Tone::Primary, leading_icon: None, trailing_icon: Some(icon()),
        };
        rt(v, |m| m.into_component("l").unwrap(), Link::try_from_component);
    }
    ...
    fn menu_button_roundtrip() {
        let v = MenuButton {
            ...
            items: vec![MenuItem {
                id: "a".into(), label: lit("A"),
                icon: None, badge: None, shortcut: None,
                danger: false, disabled: None, divider_after: false,
            }],
            placement: MenuPlacement::BottomStart,
        };
        rt(v, |m| m.into_component("mb").unwrap(), MenuButton::try_from_component);
    }

    #[test]
    fn menu_roundtrip() {
        let v = Menu { items: vec![], search: true };
        rt(v, |m| m.into_component("m").unwrap(), Menu::try_from_component);
    }
    ...
    fn action_bar_roundtrip() {
        let v = ActionBar {
            ...
            divider_between: true, sticky: false,
        };
        rt(v, |m| m.into_component("ab").unwrap(), ActionBar::try_from_component);
    }
    ...
    fn segmented_control_roundtrip() {
        let v = SegmentedControl {
            ...
            options: vec![SegmentOption {
                ...
                label: Some(lit("A")), icon: None, badge: None,
            }],
            size: SegmentSize::Md, full_width: false,
        };
        rt(v, |m| m.into_component("sc").unwrap(), SegmentedControl::try_from_component);
    }
    ...
    fn filter_chips_roundtrip() {
        let v = FilterChips {
            chips: vec![FilterChipDef {
                id: "a".into(), label: lit("A"),
                icon: None, badge: None, count_path: None,
            }],
            selected_ids: p("filters"),
            mode: FilterChipsMode::Multi, clearable: true,
        };
        rt(v, |m| m.into_component("fc").unwrap(), FilterChips::try_from_component);
    }
    ...
    fn wizard_footer_roundtrip() {
        let v = WizardFooter {
            ...
            cancel_action: None, skip_action: None,
            extra_actions: vec![],
        };
        rt(v, |m| m.into_component("wf").unwrap(), WizardFooter::try_from_component);
    }

    #[test]
    fn fab_roundtrip() {
        let v = Fab {
            icon: icon(), tone: Tone::Primary, size: FabSize::Lg,
            position: FabPosition::BottomRight, label: Some(lit("Add")),
        };
        rt(v, |m| m.into_component("fab").unwrap(), Fab::try_from_component);
    }
    ...
    fn non_button() -> Component {
        ...
        .into_component("not-a-button").unwrap()
    }
    ...
    fn action_bar_rejects_non_button_ref() {
        let bad = ActionBar {
            ...
            divider_between: false, sticky: false,
        };
    ...
    fn wizard_footer_rejects_non_button_ref() {
        let bad = WizardFooter {
            ...
            next_action: None, cancel_action: None, skip_action: None,
            extra_actions: vec![],
        };
