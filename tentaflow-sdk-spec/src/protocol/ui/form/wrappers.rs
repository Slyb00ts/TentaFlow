...
pub enum FormValidator {
    AllRequired { field_ids: Vec<String> },
    AnyRequired { field_ids: Vec<String>, error_message: BindRef },
    Match { field_a: String, field_b: String },
    ...
    fn encode<W: minicbor::encode::Write>(
        ...
        match self {
            FormValidator::AllRequired { field_ids } => {
                ...
                for s in field_ids { e.str(s)?; }
            }
            FormValidator::AnyRequired { field_ids, error_message } => {
                e.map(3)?;
...
impl<'b, C> Decode<'b, C> for FormValidator {
    fn decode(d: &mut Decoder<'b>, ctx: &mut C) -> Result<Self, minicbor::decode::Error> {
        ...
        for _ in 0..len {
            ...
            match k {
                "kind" => {
                    if seen_kind { return Err(minicbor::decode::Error::message("FormValidator: duplicate key 'kind'")); }
                    seen_kind = true;
                    kind = Some(d.str()?.to_string());
                }
                "field_ids" => {
                    if seen_field_ids { return Err(minicbor::decode::Error::message("FormValidator: duplicate key 'field_ids'")); }
                    seen_field_ids = true;
                    let n = d.array()?.ok_or_else(|| minicbor::decode::Error::message("indefinite-length array forbidden"))?;
                    let mut out = Vec::with_capacity(n as usize);
                    for _ in 0..n { out.push(d.str()?.to_string()); }
                    field_ids = Some(out);
                }
                "error_message" => {
                    if seen_error_message { return Err(minicbor::decode::Error::message("FormValidator: duplicate key 'error_message'")); }
                    seen_error_message = true;
                    error_message = Some(BindRef::decode(d, ctx)?);
                }
                "field_a" => {
                    if seen_field_a { return Err(minicbor::decode::Error::message("FormValidator: duplicate key 'field_a'")); }
                    seen_field_a = true;
                    field_a = Some(d.str()?.to_string());
                }
                "field_b" => {
                    if seen_field_b { return Err(minicbor::decode::Error::message("FormValidator: duplicate key 'field_b'")); }
                    seen_field_b = true;
                    field_b = Some(d.str()?.to_string());
                }
                "id" => {
                    if seen_id { return Err(minicbor::decode::Error::message("FormValidator: duplicate key 'id'")); }
                    seen_id = true;
                    id = Some(d.str()?.to_string());
                }
                "params" => {
                    if seen_params { return Err(minicbor::decode::Error::message("FormValidator: duplicate key 'params'")); }
                    seen_params = true;
        ...
        let kind = kind.ok_or_else(|| minicbor::decode::Error::message("FormValidator: missing 'kind'"))?;
        match kind.as_str() {
            "all_required" => {
                if seen_error_message || seen_field_a || seen_field_b || seen_id || seen_params {
                    return Err(minicbor::decode::Error::message("FormValidator::all_required: unexpected key"));
                }
                Ok(FormValidator::AllRequired {
                    field_ids: field_ids.ok_or_else(|| minicbor::decode::Error::message("FormValidator::all_required: missing 'field_ids'"))?,
                })
            }
            "any_required" => {
                if seen_field_a || seen_field_b || seen_id || seen_params {
                    return Err(minicbor::decode::Error::message("FormValidator::any_required: unexpected key"));
                }
                Ok(FormValidator::AnyRequired {
                    field_ids: field_ids.ok_or_else(|| minicbor::decode::Error::message("FormValidator::any_required: missing 'field_ids'"))?,
                    error_message: error_message.ok_or_else(|| minicbor::decode::Error::message("FormValidator::any_required: missing 'error_message'"))?,
                })
            }
            "match" => {
                if seen_field_ids || seen_error_message || seen_id || seen_params {
                    return Err(minicbor::decode::Error::message("FormValidator::match: unexpected key"));
                }
                Ok(FormValidator::Match {
                    field_a: field_a.ok_or_else(|| minicbor::decode::Error::message("FormValidator::match: missing 'field_a'"))?,
                    field_b: field_b.ok_or_else(|| minicbor::decode::Error::message("FormValidator::match: missing 'field_b'"))?,
                })
            }
            "custom" => {
                if seen_field_ids || seen_error_message || seen_field_a || seen_field_b {
                    return Err(minicbor::decode::Error::message("FormValidator::custom: unexpected key"));
                }
                Ok(FormValidator::Custom {
                    id: id.ok_or_else(|| minicbor::decode::Error::message("FormValidator::custom: missing 'id'"))?,
                    params,
                })
            }
...
impl FormField {
    ...
    pub fn into_component(self, id: impl Into<String>) -> Result<Component, IntoComponentError> {
        ...
        if let Some(v) = &self.hint { e.push((1, encode_to_value(v)?)); }
        if let Some(v) = &self.error { e.push((2, encode_to_value(v)?)); }
    ...
    pub fn try_from_component(c: &Component) -> Result<Self, minicbor::decode::Error> {
        ...
        let mut label = None; let mut hint = None; let mut error = None;
        let mut required = None; let mut child = None; let mut layout = None;
...
impl FormGroup {
    ...
    pub fn into_component(self, id: impl Into<String>) -> Result<Component, IntoComponentError> {
        ...
        if let Some(v) = &self.description { e.push((1, encode_to_value(v)?)); }
        e.push((2, encode_to_value(&self.collapsible)?));
        if let Some(v) = &self.expanded { e.push((3, encode_to_value(v)?)); }
    ...
    pub fn try_from_component(c: &Component) -> Result<Self, minicbor::decode::Error> {
        ...
        let mut title = None; let mut description = None;
        let mut collapsible = None; let mut expanded = None;
        let mut children = None; let mut spacing = None;
        for (k, v) in &c.fields.0 {
            match k {
...
impl FormSection {
    ...
    pub fn try_from_component(c: &Component) -> Result<Self, minicbor::decode::Error> {
        ...
        let mut title = None; let mut description = None;
        let mut children = None; let mut spacing = None; let mut divider_top = None;
        for (k, v) in &c.fields.0 {
            match k {
                0 => title = Some(decode_from_value(v)?),
                1 => description = Some(decode_from_value(v)?),
...
impl Form {
    ...
    pub fn try_from_component(c: &Component) -> Result<Self, minicbor::decode::Error> {
        ...
        let mut children = None; let mut scope_id = None;
        let mut validators = None; let mut prevent_default_submit = None;
        let mut layout = None; let mut disabled = None;
        for (k, v) in &c.fields.0 {
            match k {
        ...
        Ok(Form {
            ...
            prevent_default_submit: prevent_default_submit.ok_or_else(|| missing_field("Form", "prevent_default_submit"))?,
