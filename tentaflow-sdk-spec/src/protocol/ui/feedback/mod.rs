...
mod tests {
    ...
    fn icon() -> IconRef {
        IconRef::Named { name: IconName::Check, size: None, tone: None }
    }
    fn non_button() -> Component {
        Fab {
            icon: icon(), tone: Tone::Primary, size: FabSize::Md,
            position: FabPosition::Inline, label: None,
        }
        .into_component("not-btn").unwrap()
    }
    fn sample_button() -> Component {
        Button {
            variant: ButtonVariant::Primary, tone: Tone::Primary,
            label: lit("OK"), icon_leading: None, icon_trailing: None,
            size: ButtonSize::Md, full_width: false,
            disabled: None, loading: None, density: Density::Comfortable,
        }
        .into_component("b").unwrap()
    }
    ...
    fn alert_roundtrip() {
        let v = Alert {
            tone: Tone::Warning, variant: AlertVariant::Soft,
            icon: Some(icon()), title: Some(lit("Heads up")),
            message: lit("Saved"), actions: Some(vec![sample_button()]),
            dismissible: true,
        };
        rt(v, |m| m.into_component("a").unwrap(), Alert::try_from_component);
    }

    #[test]
    fn alert_rejects_non_button_action_on_encode() {
        let v = Alert {
            tone: Tone::Critical, variant: AlertVariant::Filled, icon: None,
            title: None, message: lit("x"),
            actions: Some(vec![non_button()]), dismissible: false,
        };
    ...
    fn alert_rejects_non_button_action_on_decode() {
        ...
        let good = Alert {
            tone: Tone::Info, variant: AlertVariant::Default, icon: None,
            title: None, message: lit("ok"),
            actions: Some(vec![sample_button()]), dismissible: false,
        }
        .into_component("a").unwrap();
    ...
    fn banner_rejects_non_button_action_on_encode() {
        let v = Banner {
            tone: Tone::Info, icon: None, message: lit("x"),
            action: Some(non_button()), dismissible: false,
    ...
    fn banner_rejects_non_button_action_on_decode() {
        let good = Banner {
            tone: Tone::Info, icon: None, message: lit("ok"),
            action: Some(sample_button()), dismissible: false,
            position: BannerPosition::Top,
        }
        .into_component("bn").unwrap();
        let mut tampered = good;
        for (k, v) in tampered.fields.0.iter_mut() {
            if *k == 3 {
    ...
    fn gate_screen_rejects_non_button_on_decode() {
        let good = GateScreen {
            icon: icon(), title: lit("t"), message: lit("m"),
            actions: vec![sample_button()],
            variant: GateVariant::AuthRequired,
        }
        .into_component("gs").unwrap();
        let mut tampered = good;
        let bad_actions: Vec<Component> = vec![non_button()];
        for (k, v) in tampered.fields.0.iter_mut() {
            if *k == 3 {
    ...
    fn banner_roundtrip() {
        let v = Banner {
            tone: Tone::Info, icon: None,
            message: lit("Welcome"),
            action: Some(sample_button()),
            dismissible: true, position: BannerPosition::Top,
        };
        rt(v, |m| m.into_component("bn").unwrap(), Banner::try_from_component);
    }

    #[test]
    fn callout_roundtrip() {
        let v = Callout {
            tone: Tone::Neutral, icon: None,
            title: Some(lit("Note")),
            content: vec![],
        };
        rt(v, |m| m.into_component("co").unwrap(), Callout::try_from_component);
    }

    #[test]
    fn toast_roundtrip() {
        let v = Toast {
            tone: Tone::Success, title: lit("Saved"),
            body: Some(lit("All changes stored")),
            icon: None, action_label: Some("Undo".into()),
            action_id: Some("undoSave".into()),
        };
        rt(v, |m| m.into_component("t").unwrap(), Toast::try_from_component);
    }

    #[test]
    fn hint_roundtrip() {
        let v = Hint { content: lit("Try /search"), icon: None, tone: Tone::Neutral };
        rt(v, |m| m.into_component("h").unwrap(), Hint::try_from_component);
    }
    ...
    fn skeleton_roundtrip() {
        let v = Skeleton {
            ...
            height: None, animate: true, lines: 3,
        };
        rt(v, |m| m.into_component("sk").unwrap(), Skeleton::try_from_component);
    }

    #[test]
    fn spinner_roundtrip() {
        let v = Spinner {
            size: SpinnerSize::Md, tone: Tone::Primary,
            label: Some(lit("Loading")), variant: SpinnerVariant::Ring,
        };
        rt(v, |m| m.into_component("sp").unwrap(), Spinner::try_from_component);
    }
    ...
    fn loading_bar_roundtrip() {
        ...
        rt(v, |m| m.into_component("lb").unwrap(), LoadingBar::try_from_component);
    }

    #[test]
    fn modal_roundtrip() {
        let v = Modal {
            title: lit("Edit"), subtitle: None,
    ...
    fn drawer_roundtrip() {
        let v = Drawer {
            side: DrawerSide::Right, size: DrawerSize::Lg,
            title: Some(lit("Filters")),
            body_slot: "body".into(), footer_slot: None, dismissible: true,
        };
        rt(v, |m| m.into_component("d").unwrap(), Drawer::try_from_component);
    }

    #[test]
    fn popover_roundtrip() {
        let v = Popover {
            anchor_id: "btn1".into(), body_slot: "p-body".into(),
            placement: PopoverPlacement::BottomStart,
            dismissible: true, arrow: true,
        };
        rt(v, |m| m.into_component("po").unwrap(), Popover::try_from_component);
    }

    #[test]
    fn sheet_roundtrip() {
        let v = Sheet {
            title: None, body_slot: "s-body".into(), footer_slot: None,
            detents: vec![SheetDetent::Small, SheetDetent::Medium, SheetDetent::Large],
            current_detent: Some(lit("medium")), dismissible: true,
        };
        rt(v, |m| m.into_component("sh").unwrap(), Sheet::try_from_component);
    }

    #[test]
    fn gate_screen_roundtrip() {
        let v = GateScreen {
            icon: icon(), title: lit("Permission denied"),
            message: lit("You don't have access"),
            actions: vec![sample_button()],
            variant: GateVariant::PermissionDenied,
        };
        rt(v, |m| m.into_component("gs").unwrap(), GateScreen::try_from_component);
    }

    #[test]
    fn gate_screen_rejects_non_button_on_encode() {
        let v = GateScreen {
            icon: icon(), title: lit("x"), message: lit("y"),
            actions: vec![non_button()], variant: GateVariant::AuthRequired,
        };
    ...
    fn confirmation_dialog_roundtrip() {
        let v = ConfirmationDialog {
            title: lit("Delete?"), message: lit("This cannot be undone"),
            icon: Some(icon()), tone: Tone::Critical,
            confirm_label: lit("Delete"), cancel_label: lit("Cancel"),
            destructive: true, require_typing: Some("DELETE".into()),
        };
        rt(v, |m| m.into_component("cd").unwrap(), ConfirmationDialog::try_from_component);
    }
    ...
    fn offline_banner_roundtrip() {
        ...
        rt(v, |m| m.into_component("ob").unwrap(), OfflineBanner::try_from_component);
    }
