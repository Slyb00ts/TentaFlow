...
// Oferuje API do embeddings, chat completion, RAG, TTS, STT i ingestion.
//
// JAK DZIAŁA:
...
/// Zapewnia dostęp do embeddings, chat completion, RAG, TTS, STT i ingestion dokumentów.
/// </summary>
/// <remarks>
...
public sealed class TentaFlowClient : IDisposable
{
    ...
    /// <summary>
    /// Wysyła zapytanie RAG (Retrieval Augmented Generation).
    /// </summary>
    /// <param name="query">Tekst zapytania.</param>
    /// <param name="topK">Liczba wyników do zwrócenia.</param>
    /// <param name="minSimilarity">Minimalny próg podobieństwa (0.0-1.0).</param>
    /// <param name="searchModes">Tryby wyszukiwania (FTS, Vector, HiRAG, GSW) - można łączyć. Domyślnie VectorSearch.</param>
    /// <param name="useReranking">Czy użyć rerankingu po zebraniu wyników (null = domyślnie false).</param>
    /// <param name="requiresLlm">Czy przetworzyć wyniki przez LLM (null = domyślnie false).</param>
    /// <param name="requiresAudio">Czy wygenerować audio TTS (null = domyślnie false).</param>
    /// <returns>Wynik RAG z kontekstem i metadanymi.</returns>
    public RagResult Rag(
        string query,
        uint topK = 5,
        float minSimilarity = 0.7f,
        SearchMode? searchModes = null,
        bool? useReranking = null,
        bool? requiresLlm = null,
        bool? requiresAudio = null)
    {
        ThrowIfDisposed();
        ArgumentException.ThrowIfNullOrEmpty(query);

        // Konwersja parametrów na wartości FFI
        uint searchModesFlags = searchModes.HasValue ? (uint)searchModes.Value : 0;
        int useRerankingInt = useReranking.HasValue ? (useReranking.Value ? 1 : 0) : -1;
        int requiresLlmInt = requiresLlm.HasValue ? (requiresLlm.Value ? 1 : 0) : -1;
        int requiresAudioInt = requiresAudio.HasValue ? (requiresAudio.Value ? 1 : 0) : -1;

        var result = NativeMethods.tentaflow_rag(
            _clientHandle,
            query,
            topK,
            minSimilarity,
        ...
        try
        {
            ...
            {
                var error = Marshal.PtrToStringUTF8(result.Error);
                throw new TentaFlowException($"RAG error: {error}");
            }

            // Marshal chunks array
            var chunks = new List<RagChunkInfo>();
            if (result.Chunks != nint.Zero && result.ChunksCount > 0)
            {
                var chunkSize = Marshal.SizeOf<RagChunkInfoNative>();
                for (int i = 0; i < result.ChunksCount; i++)
                {
                    var chunkPtr = result.Chunks + (i * chunkSize);
                    var chunkNative = Marshal.PtrToStructure<RagChunkInfoNative>(chunkPtr);

                    // Marshal documents array for this chunk
                    var documents = new List<ChunkDocument>();
                    if (chunkNative.Documents != nint.Zero && chunkNative.DocumentsCount > 0)
                    {
                        var docSize = Marshal.SizeOf<ChunkDocumentNative>();
                        for (int j = 0; j < chunkNative.DocumentsCount; j++)
                        {
                            var docPtr = chunkNative.Documents + (j * docSize);
                            var docNative = Marshal.PtrToStructure<ChunkDocumentNative>(docPtr);

                            // Marshal metadata for this document
                            var metadata = new Dictionary<string, string>();
                            if (docNative.Metadata != nint.Zero && docNative.MetadataCount > 0)
                            {
                                var kvSize = Marshal.SizeOf<KeyValuePairNative>();
                                for (int k = 0; k < docNative.MetadataCount; k++)
                                {
                                    var kvPtr = docNative.Metadata + (k * kvSize);
                                    var kvNative = Marshal.PtrToStructure<KeyValuePairNative>(kvPtr);
                                    var key = Marshal.PtrToStringUTF8(kvNative.Key) ?? string.Empty;
                                    var value = Marshal.PtrToStringUTF8(kvNative.Value) ?? string.Empty;
                            ...
                            {
                                DocId = Marshal.PtrToStringUTF8(docNative.DocId) ?? string.Empty,
                                Metadata = metadata
                            });
                        }
                    }

                    chunks.Add(new RagChunkInfo
                    {
                        ChunkId = Marshal.PtrToStringUTF8(chunkNative.ChunkId) ?? string.Empty,
                        ChunkText = Marshal.PtrToStringUTF8(chunkNative.ChunkText) ?? string.Empty,
                        SourceFile = Marshal.PtrToStringUTF8(chunkNative.SourceFile) ?? string.Empty,
                        SourceType = Marshal.PtrToStringUTF8(chunkNative.SourceType) ?? string.Empty,
                        SimilarityScore = chunkNative.SimilarityScore,
                        Rank = chunkNative.Rank,
                        ChunkIndex = chunkNative.ChunkIndex,
                        Documents = documents
                    });
                }
            }
            ...
            {
                Response = Marshal.PtrToStringUTF8(result.Response) ?? string.Empty,
                ChunksFound = result.ChunksFound,
                RequiresLlm = result.RequiresLlm,
                Chunks = chunks
            };
        }
        finally
        {
            NativeMethods.tentaflow_free_rag(result);
        }
    }
    ...
    /// <summary>
    /// Dodaje dokument tekstowy do RAG (indeksowanie).
    /// </summary>
    /// <param name="documentId">Unikalny identyfikator dokumentu.</param>
    /// <param name="text">Treść tekstowa dokumentu.</param>
    /// <param name="metadata">Opcjonalne pary klucz-wartość metadanych.</param>
    /// <returns>Wynik indeksowania ze statystykami.</returns>
        ...
        IDictionary<string, string>? metadata = null)
    {
        ...
        try
        {
            var (metaPtr, metaCount) = MarshalMetadata(metadata, handles);

            var result = NativeMethods.tentaflow_ingest_text(
                _clientHandle,
                documentId,
                text,
                metaPtr,
            ...
            try
            {
                return ProcessIngestResult(result);
            }
            finally
            {
                NativeMethods.tentaflow_free_ingest(result);
            }
        }
        finally
        {
    ...
    /// <summary>
    /// Dodaje plik binarny do RAG (indeksowanie).
    /// </summary>
    /// <param name="documentId">Unikalny identyfikator dokumentu.</param>
    /// <param name="filename">Nazwa pliku z rozszerzeniem (dla detekcji MIME).</param>
        ...
        IDictionary<string, string>? metadata = null)
    {
        ...
        try
        {
            ...
            var result = NativeMethods.tentaflow_ingest_file(
                _clientHandle,
                documentId,
                filename,
                dataHandle.AddrOfPinnedObject(),
    ...
    private static (nint ptr, int count) MarshalMetadata(
        ...
        List<GCHandle> handles)
    {
        ...
        var entries = new MetadataEntryNative[metadata.Count];
        int i = 0;
        foreach (var kvp in metadata)
        {
            entries[i] = new MetadataEntryNative
            {
                Key = MarshalString(kvp.Key, handles),
                Value = MarshalString(kvp.Value, handles)
            };
            i++;
        }

        var entriesHandle = GCHandle.Alloc(entries, GCHandleType.Pinned);
        handles.Add(entriesHandle);

        return (entriesHandle.AddrOfPinnedObject(), metadata.Count);
    }

    private static IngestResult ProcessIngestResult(IngestResultNative result)
    {
        if (result.Error != nint.Zero)
        {
            var error = Marshal.PtrToStringUTF8(result.Error);
            throw new TentaFlowException($"Ingest error: {error}");
        }

        return new IngestResult
        {
            DocumentId = Marshal.PtrToStringUTF8(result.DocumentId) ?? string.Empty,
