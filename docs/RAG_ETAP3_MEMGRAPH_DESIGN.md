...
## A ✅ — GUI addona RAG + uniwersalny upload platformy. Panel main NavTabs 5 zakładek (Kolekcje/Dokumenty+ingest/Czat+cytaty/Graf explorer/Konflikty D7) w Rust SDK UI (render przez sdk-runtime); read-side tooly (list/create/delete/status/conflicts/graph_explore/resolve_escalated odwracalny z guard members_rev). Uniwersalny upload plików dla paneli addonów: protokół AddonDocumentBody chunked z serde_bytes (CBOR byte-string nie array-of-ints!), backend zapis do document store instancji (izolacja org z auth), front per-chunk slice (bez OOM) sekwencyjny+abort, upload_complete→ingest_document. panel_epoch z akcji (wstrzyknięty przez ui_channel, źródło prawdy przy pulowaniu instancji), pola formularza per-sesja (f:user:epoch:field, Ephemeral). Codex GO (upload 2 rundy, panel 5 rund). 

# CAŁOŚĆ UKOŃCZONA: etapy 0-3 (RAG→GraphRAG→MemGraphRAG D1-D6) + A (GUI+upload). 42 commity, wszystkie codex-GO.
