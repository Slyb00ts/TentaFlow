...
- Transport iroh QUIC obsługuje `IrohMeshManager` (`tentaflow-core/src/mesh/iroh_manager.rs`). `net/iroh/relay.rs::load_relay_url` wybiera poprawny URL kolejno z DB `settings.mesh.iroh_relay_url`, konfiguracji `[mesh] iroh_relay_url`, a następnie domyślnego `https://relay.nextapp.pl`. `net/iroh/endpoint.rs` ustawia `RelayMode::Custom` dla otrzymanego URL; `None` wyłącza relay (`RelayMode::Disabled`). Nie korzysta wtedy z presetu N0.
...
- Lista `MeshNodeList` pokazuje lokalny węzeł, peery wykryte przez lokalny mDNS oraz aktywnie zaufane według lokalnego `MeshSecurity`. Znacznik `is_lan_discovered` jest efemeryczny i nie pochodzi z gossip, odtworzonych kontaktów ani samego połączenia przychodzącego. Filtr obejmuje zarówno rejestr, jak i `peer_store`; zaufany peer może pozostać widoczny offline lub korzystać z relay. Relay i DHT służą transportowi oraz odnajdywaniu znanych identyfikatorów, nie enumeracji obcych urządzeń na liście.
- Zaufanie przekazane przez `TrustedKeysSync` nadal daje widoczność: po sparowaniu A–B oraz A–C synchronizacja kluczy pozwala B i C ufać sobie bez dodatkowego parowania. Filtr nie wymaga `approved_by` równego lokalnemu ID; odrzuca kontakty bez aktywnego zaufania, a nie zaufanie propagowane w mesh.
