...
pub fn handle_get(pool: &DbPool, id: i64) -> Result<(u16, String)> {
    match db::repository::get_prompt(pool, id)? {
        ...
        None => Ok((404, format!(r#"{{"error":"Prompt o id {} nie istnieje"}}"#, id))),
    }
}

/// POST /api/prompts - utworz nowy prompt
pub fn handle_create(pool: &DbPool, body: &[u8]) -> Result<(u16, String)> {
    let req: CreatePromptRequest = serde_json::from_slice(body)
    ...
    if !ALLOWED_PROMPT_TYPES.contains(&req.prompt_type.as_str()) {
        return Ok((400, format!(
            r#"{{"error":"Niedozwolona wartosc prompt_type '{}'. Dozwolone: {}"}}"#,
            req.prompt_type,
...
pub fn handle_update(pool: &DbPool, id: i64, body: &[u8]) -> Result<(u16, String)> {
    ...
    if existing.is_none() {
        return Ok((404, format!(r#"{{"error":"Prompt o id {} nie istnieje"}}"#, id)));
    }

    let req: UpdatePromptRequest = serde_json::from_slice(body)
        .map_err(|e| anyhow::anyhow!("Niepoprawny JSON: {}", e))?;
