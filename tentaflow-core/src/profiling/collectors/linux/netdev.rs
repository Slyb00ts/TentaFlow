#![cfg_attr(not(target_os = "linux"), allow(dead_code))]
// =============================================================================
// File: collectors/linux/netdev.rs — Network throughput / packet rate collector
// reading /proc/net/dev at 10 Hz. Wytwarza TimelineEvent::NetworkSample per
// interface. Odblokowuje mockup #04 KPI Network + mockup #14 Network in range.
// =============================================================================

use std::collections::HashMap;
use std::fs;
#[cfg(target_os = "linux")]
use std::io::Write;
use std::path::PathBuf;
use std::sync::atomic::{AtomicBool, AtomicU64, Ordering};
use std::sync::Arc;
#[cfg(target_os = "linux")]
use std::thread;
use std::thread::JoinHandle;
use std::time::{Duration, Instant};

use tentaflow_protocol::profiling::{
    ClockSamples, ElevationRequirement, EventCategory, EventPayload, TimelineEvent,
};

use crate::profiling::collectors::{
    CollectorCapability, CollectorError, CollectorParser, FrameInterner, NameInterner,
    PlatformSet, ProbeResult, ProfileCollector, RawCapture, RunningCollector, SessionCtx,
};

const COLLECTOR_ID: &str = "linux.netdev";
const CSV_FILENAME: &str = "netdev.csv";
const SAMPLE_PERIOD: Duration = Duration::from_millis(100);

/// Network throughput collector via /proc/net/dev.
pub struct LinuxNetdevCollector {
    capability: CollectorCapability,
    id: String,
}

impl LinuxNetdevCollector {
    pub fn new() -> Self {
        Self {
            capability: CollectorCapability {
                categories: vec![EventCategory::NetworkSample],
                elevation: ElevationRequirement::None,
                platforms: PlatformSet::from_flags(
                    PlatformSet::LINUX_X64 | PlatformSet::LINUX_ARM64,
                ),
                vendor: None,
                description:
                    "Per-interface network throughput and packet rate, sampled from /proc/net/dev at 10 Hz.",
            },
            id: COLLECTOR_ID.to_string(),
        }
    }
}

impl Default for LinuxNetdevCollector {
    fn default() -> Self {
        Self::new()
    }
}

impl ProfileCollector for LinuxNetdevCollector {
    fn id(&self) -> &str {
        &self.id
    }
    fn capability(&self) -> &CollectorCapability {
        &self.capability
    }
    fn probe(&self) -> ProbeResult {
        #[cfg(target_os = "linux")]
        {
            match fs::metadata("/proc/net/dev") {
                Ok(_) => ProbeResult::Available { version: None },
                Err(e) => ProbeResult::Unavailable {
                    reason: format!("/proc/net/dev not readable: {e}"),
                },
            }
        }
        #[cfg(not(target_os = "linux"))]
        {
            ProbeResult::Unavailable {
                reason: "linux.netdev is Linux-only".into(),
            }
        }
    }

    #[cfg(target_os = "linux")]
    fn start(&self, ctx: SessionCtx) -> Result<Box<dyn RunningCollector>, CollectorError> {
        fs::create_dir_all(&ctx.output_dir)?;
        let csv_path = ctx.output_dir.join(CSV_FILENAME);
        let stop_flag = Arc::new(AtomicBool::new(false));
        let samples_observed = Arc::new(AtomicU64::new(0));
        let started_at = Instant::now();
        let start_clock_ns = read_monotonic_ns();

        let stop_t = stop_flag.clone();
        let samples_t = samples_observed.clone();
        let csv_t = csv_path.clone();
        let started_t = started_at;

        let handle = thread::Builder::new()
            .name("tf-netdev-collector".into())
            .spawn(move || {
                if let Err(e) = polling_loop(stop_t, samples_t, csv_t, started_t) {
                    eprintln!("linux.netdev polling loop ended: {e}");
                }
            })
            .map_err(|e| CollectorError::Spawn(format!("netdev thread spawn: {e}")))?;

        Ok(Box::new(LinuxNetdevRunning {
            id: COLLECTOR_ID.to_string(),
            output_dir: ctx.output_dir.clone(),
            csv_path,
            stop_flag,
            samples_observed,
            handle: Some(handle),
            started_at,
            start_clock_ns,
        }))
    }

    #[cfg(not(target_os = "linux"))]
    fn start(&self, _ctx: SessionCtx) -> Result<Box<dyn RunningCollector>, CollectorError> {
        Err(CollectorError::Custom("linux.netdev is Linux-only".into()))
    }
}

pub struct LinuxNetdevRunning {
    id: String,
    output_dir: PathBuf,
    csv_path: PathBuf,
    stop_flag: Arc<AtomicBool>,
    samples_observed: Arc<AtomicU64>,
    handle: Option<JoinHandle<()>>,
    started_at: Instant,
    start_clock_ns: u64,
}

impl RunningCollector for LinuxNetdevRunning {
    fn collector_id(&self) -> &str {
        &self.id
    }
    fn stop(mut self: Box<Self>) -> Result<RawCapture, CollectorError> {
        self.stop_flag.store(true, Ordering::Relaxed);
        if let Some(h) = self.handle.take() {
            let _ = h.join();
        }
        let end_clock_ns = read_monotonic_ns();
        let end_session_ns = self.started_at.elapsed().as_nanos() as u64;

        let mut metadata: HashMap<String, String> = HashMap::new();
        metadata.insert("source".into(), "/proc/net/dev".into());
        metadata.insert("sample_period_ms".into(), "100".into());

        let artifacts = if self.csv_path.exists() {
            vec![self.csv_path.clone()]
        } else {
            Vec::new()
        };

        Ok(RawCapture {
            artifacts,
            metadata,
            clock_samples: ClockSamples {
                collector_id: COLLECTOR_ID.to_string(),
                pairs: vec![(self.start_clock_ns, 0), (end_clock_ns, end_session_ns)],
            },
            samples_observed: self.samples_observed.load(Ordering::Relaxed),
        })
    }
    fn abort(mut self: Box<Self>) {
        self.stop_flag.store(true, Ordering::Relaxed);
        if let Some(h) = self.handle.take() {
            let _ = h.join();
        }
        let _ = fs::remove_dir_all(&self.output_dir);
    }
}

#[cfg(target_os = "linux")]
fn polling_loop(
    stop_flag: Arc<AtomicBool>,
    samples_observed: Arc<AtomicU64>,
    csv_path: PathBuf,
    started_at: Instant,
) -> Result<(), CollectorError> {
    let mut file = fs::File::create(&csv_path)?;
    writeln!(file, "timestamp_ns,iface,rx_bps,tx_bps,rx_pps,tx_pps")?;

    let mut prev = read_proc_net_dev()?;
    let mut prev_t = Instant::now();
    while !stop_flag.load(Ordering::Relaxed) {
        thread::sleep(SAMPLE_PERIOD);
        if stop_flag.load(Ordering::Relaxed) {
            break;
        }
        let now = read_proc_net_dev()?;
        let dt_s = prev_t.elapsed().as_secs_f64().max(1e-3);
        let ts_ns = started_at.elapsed().as_nanos() as u64;
        for (iface, cur) in &now {
            let Some(p) = prev.get(iface) else {
                continue;
            };
            let rx_bytes = cur.rx_bytes.saturating_sub(p.rx_bytes);
            let tx_bytes = cur.tx_bytes.saturating_sub(p.tx_bytes);
            let rx_pkts = cur.rx_pkts.saturating_sub(p.rx_pkts);
            let tx_pkts = cur.tx_pkts.saturating_sub(p.tx_pkts);
            let rx_bps = ((rx_bytes as f64 * 8.0) / dt_s) as u64;
            let tx_bps = ((tx_bytes as f64 * 8.0) / dt_s) as u64;
            let rx_pps = ((rx_pkts as f64) / dt_s) as u32;
            let tx_pps = ((tx_pkts as f64) / dt_s) as u32;
            writeln!(file, "{ts_ns},{iface},{rx_bps},{tx_bps},{rx_pps},{tx_pps}")?;
            samples_observed.fetch_add(1, Ordering::Relaxed);
        }
        prev = now;
        prev_t = Instant::now();
    }
    file.flush()?;
    Ok(())
}

#[derive(Clone, Copy, Default)]
struct IfaceCounters {
    rx_bytes: u64,
    tx_bytes: u64,
    rx_pkts: u64,
    tx_pkts: u64,
}

#[cfg(target_os = "linux")]
fn read_proc_net_dev() -> Result<HashMap<String, IfaceCounters>, CollectorError> {
    let s = fs::read_to_string("/proc/net/dev")?;
    Ok(parse_proc_net_dev(&s))
}

fn parse_proc_net_dev(text: &str) -> HashMap<String, IfaceCounters> {
    let mut out = HashMap::new();
    for line in text.lines().skip(2) {
        // Format: "  iface: rx_bytes rx_pkts rx_errs ... tx_bytes tx_pkts tx_errs ..."
        let line = line.trim_start();
        let Some((iface, rest)) = line.split_once(':') else {
            continue;
        };
        let cols: Vec<&str> = rest.split_whitespace().collect();
        // Need: rx_bytes (col 0), rx_pkts (col 1), tx_bytes (col 8), tx_pkts (col 9).
        if cols.len() < 10 {
            continue;
        }
        let parse = |idx: usize| -> u64 { cols[idx].parse().unwrap_or(0) };
        out.insert(
            iface.trim().to_string(),
            IfaceCounters {
                rx_bytes: parse(0),
                rx_pkts: parse(1),
                tx_bytes: parse(8),
                tx_pkts: parse(9),
            },
        );
    }
    out
}

#[cfg(target_os = "linux")]
fn read_monotonic_ns() -> u64 {
    use std::mem::MaybeUninit;
    let mut ts: MaybeUninit<libc::timespec> = MaybeUninit::uninit();
    unsafe {
        libc::clock_gettime(libc::CLOCK_MONOTONIC_RAW, ts.as_mut_ptr());
    }
    let ts = unsafe { ts.assume_init() };
    (ts.tv_sec as u64) * 1_000_000_000 + (ts.tv_nsec as u64)
}

#[cfg(not(target_os = "linux"))]
fn read_monotonic_ns() -> u64 {
    0
}

/// Parser — interns iface names + emits NetworkSample events.
pub struct LinuxNetdevParser;

impl CollectorParser for LinuxNetdevParser {
    fn parse(
        &self,
        raw: RawCapture,
        _ctx: &SessionCtx,
        names: &mut NameInterner,
        _frames: &mut FrameInterner,
    ) -> Result<Vec<TimelineEvent>, CollectorError> {
        let Some(csv_path) = raw.artifacts.first() else {
            return Ok(Vec::new());
        };
        let content = match fs::read_to_string(csv_path) {
            Ok(s) => s,
            Err(_) => return Ok(Vec::new()),
        };
        let mut events = Vec::new();
        for (idx, line) in content.lines().enumerate() {
            if idx == 0 || line.is_empty() {
                continue; // header
            }
            let cols: Vec<&str> = line.split(',').collect();
            if cols.len() < 6 {
                continue;
            }
            let ts: u64 = match cols[0].parse() {
                Ok(v) => v,
                Err(_) => continue,
            };
            let iface = cols[1];
            let rx_bps: u64 = cols[2].parse().unwrap_or(0);
            let tx_bps: u64 = cols[3].parse().unwrap_or(0);
            let rx_pps: u32 = cols[4].parse().unwrap_or(0);
            let tx_pps: u32 = cols[5].parse().unwrap_or(0);
            let iface_name_id = names.intern(iface);
            events.push(TimelineEvent {
                source_idx: 0,
                t_start_ns: ts,
                t_end_ns: ts,
                category: EventCategory::NetworkSample,
                lane_hint: 0,
                payload: EventPayload::NetworkSample {
                    iface_name_id,
                    rx_bps,
                    tx_bps,
                    rx_pps,
                    tx_pps,
                },
            });
        }
        Ok(events)
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn collector_default_id_and_capability() {
        let c = LinuxNetdevCollector::new();
        assert_eq!(c.id(), "linux.netdev");
        let cap = c.capability();
        assert!(cap.categories.contains(&EventCategory::NetworkSample));
        assert_eq!(cap.elevation, ElevationRequirement::None);
    }

    #[test]
    fn parse_proc_net_dev_skips_headers() {
        let text = "\
Inter-|   Receive                                                |  Transmit
 face |bytes    packets errs drop fifo frame compressed multicast|bytes    packets errs drop fifo colls carrier compressed
    lo: 1234567   1234    0    0    0     0          0         0  1234567    1234    0    0    0     0       0          0
  eth0: 9876543   5678    0    0    0     0          0         0  4321098    4321    0    0    0     0       0          0
";
        let map = parse_proc_net_dev(text);
        assert_eq!(map.len(), 2);
        let lo = map.get("lo").unwrap();
        assert_eq!(lo.rx_bytes, 1234567);
        assert_eq!(lo.rx_pkts, 1234);
        assert_eq!(lo.tx_bytes, 1234567);
        let eth = map.get("eth0").unwrap();
        assert_eq!(eth.rx_bytes, 9876543);
        assert_eq!(eth.tx_bytes, 4321098);
    }
}
