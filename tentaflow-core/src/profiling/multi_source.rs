...
impl ParserRegistry {
    ...
    pub fn default_registry() -> Self {
        ...
        // Linux no-priv parsers. Moduly c::linux/c::linux_gpu sa cfg-gated do
        // target_os="linux" w collectors/mod.rs, wiec rejestracje musza byc
        // tak samo gated zeby cross-compile na Windows/macOS sie udal.
        #[cfg(target_os = "linux")]
        {
            r.register(
            ...
            r.register(
                "linux.perf.cpu_sampling".to_string(),
                Arc::new(c::linux::perf_sampling::LinuxPerfSamplingParser),
            );
            r.register(
                "linux.perf.pmu_counters".to_string(),
                Arc::new(c::linux::perf_counters::LinuxPerfCountersParser),
            );
            r.register(
            ...
            r.register(
                "linux.netdev".to_string(),
                Arc::new(c::linux::netdev::LinuxNetdevParser),
            );
            r.register(
                "linux.proc.top_processes".to_string(),
                Arc::new(c::linux::top_processes::LinuxTopProcessesParser),
            );
            r.register(
                "linux.uncore.imc".to_string(),
                Arc::new(c::linux::uncore_imc::LinuxUncoreImcParser),
            );
