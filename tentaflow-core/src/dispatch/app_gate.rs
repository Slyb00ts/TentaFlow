...
pub(crate) mod test_support {
    ...
    /// Explicit form: the caller supplies the instance id AND the manifest.
    /// `install_app_instance` is the convenience wrapper over it for the common
    /// case of a bundled manifest and a `<package>-<suffix>` id.
    pub(crate) fn install_app_instance_with_manifest(
        state: &Arc<AppState>,
        package_id: &str,
        addon_id: &str,
        manifest: &str,
