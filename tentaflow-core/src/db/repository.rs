...
const SERVICE_COLS: &str = "id, name, service_type, strategy, model_category, status, config_json, created_at, updated_at, service_uuid, node_id, pinned, paused, vram_estimate_mb";
...
pub fn list_services_with_backends(
    ...
    let mut stmt = conn.prepare(
        "SELECT s.id, s.name, s.service_type, s.strategy, s.model_category, s.status, s.config_json, s.created_at, s.updated_at, s.service_uuid, s.node_id, s.pinned, s.paused, s.vram_estimate_mb, \
    ...
    while let Some(row) = rows.next()? {
        ...
        let backend_id: Option<i64> = row.get(14)?;
        if backend_id.is_some() {
            let backend = DbServiceBackend {
                ...
                max_concurrent: row.get(18)?,
                timeout_ms: row.get(19)?,
                weight: row.get(20)?,
                model_name_override: row.get(21)?,
                health_check_path: row.get(22)?,
