...
fn seed_flow_node_templates(conn: &Connection) -> Result<()> {
    ...
    let templates: &[(&str, &str, &str, &str, &str, &str, &str)] = &[
        ...
        (
            ...
            r#"{"instance_id":"","topic":"","group":"","batch_size":1,"max_wait_ms":1000,"commit_mode":"auto_after_success","on_error":"dlq"}"#,
            "inbox",
            r#"{"properties":{"instance_id":{"type":"string","title":"Instancja TentaBus","description":"Zainstalowana i włączona instancja TentaBus. Wymagane — brak domyślnej instancji.","dynamic_enum":{"source":"bus_instances"}},"topic":{"type":"string","title":"Temat"},"group":{"type":"string","title":"Grupa konsumentów"},"batch_size":{"type":"integer","title":"Rozmiar partii","minimum":1,"maximum":1000,"default":1},"max_wait_ms":{"type":"integer","title":"Maks. czas oczekiwania (ms)","minimum":1,"maximum":60000,"default":1000},"commit_mode":{"type":"string","title":"Tryb zatwierdzania","enum":[{"value":"auto_after_success","label":"Automatycznie po sukcesie"},{"value":"at_most_once","label":"Najwyżej raz"}],"default":"auto_after_success"},"on_error":{"type":"string","title":"Przy błędzie","enum":[{"value":"dlq","label":"Do DLQ"},{"value":"skip","label":"Pomiń"},{"value":"halt","label":"Zatrzymaj subskrypcję"}],"default":"dlq"},"org_id":{"type":"string","title":"Org ID (opcjonalnie)","description":"Puste = organizacja domyślna"}},"required":["instance_id","topic","group"],"order":["instance_id","topic","group","batch_size","max_wait_ms","commit_mode","on_error","org_id"]}"#,
        ),
        (
            "bus_publish",
            "output",
            ...
            r#"{"instance_id":"","topic":"","key":"","content_type":"application/json","create_if_missing":false}"#,
            "send-horizontal",
            r#"{"properties":{"instance_id":{"type":"string","title":"Instancja TentaBus","description":"Zainstalowana i włączona instancja TentaBus. Wymagane — brak domyślnej instancji.","dynamic_enum":{"source":"bus_instances"}},"topic":{"type":"string","title":"Temat"},"key":{"type":"string","title":"Klucz partycjonowania (CEL, opcjonalnie)","description":"Wyrażenie CEL, np. payload.id"},"content_type":{"type":"string","title":"Content-Type (opcjonalnie)"},"headers":{"type":"object","title":"Nagłówki (opcjonalnie)","description":"Mapa nazwa nagłówka -> wyrażenie CEL (np. \"'application/json'\" albo payload.field). content-type z pola powyżej trafia jako domyślny nagłówek content-type; wpis tutaj o tej samej nazwie go nadpisuje.","additionalProperties":{"type":"string"}},"create_if_missing":{"type":"boolean","title":"Utwórz temat, jeśli nie istnieje","description":"Zgoda noda na automatyczne utworzenie tematu — sama nie wystarcza: musi na to pozwolić także ustawienie organizacji bus.autocreate (domyślnie włączone tylko w środowisku Dev). Gdy ustawienie jest wyłączone, publikacja do nieistniejącego tematu kończy się błędem zamiast utworzyć temat","default":false}},"required":["instance_id","topic"],"order":["instance_id","topic","key","content_type","headers","create_if_missing"]}"#,
        ),
        (
            "bus_transform",
            "transform",
            ...
            r#"{"instance_id":"","expression":""}"#,
            "shuffle",
            r#"{"properties":{"instance_id":{"type":"string","title":"Instancja TentaBus","description":"Zainstalowana i włączona instancja TentaBus. Wymagane — brak domyślnej instancji.","dynamic_enum":{"source":"bus_instances"}},"expression":{"type":"string","title":"Wyrażenie CEL","format":"textarea","placeholder":"{'wynik': payload.value}"}},"required":["instance_id","expression"],"order":["instance_id","expression"]}"#,
        ),
    ];
