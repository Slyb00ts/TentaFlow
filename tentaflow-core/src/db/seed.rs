...
fn seed_flow_node_templates(conn: &Connection) -> Result<()> {
    ...
    let templates: &[(&str, &str, &str, &str, &str, &str, &str)] = &[
        ...
        (
            ...
            "shuffle",
            r#"{"properties":{"instance_id":{"type":"string","title":"Instancja TentaBus","description":"ID zainstalowanej instancji TentaBus (tentabus-XXXXXXXX). Wymagane — brak domyślnej instancji."},"expression":{"type":"string","title":"Wyrażenie CEL","format":"textarea","placeholder":"{'wynik': payload.value}"}},"required":["instance_id","expression"],"order":["instance_id","expression"]}"#,
        ),
    ];
