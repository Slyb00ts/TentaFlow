...
mod tests {
    ...
    fn fresh_db_has_expected_default_flows() {
        ...
        assert_eq!(
            total, 11,
            "oczekiwane 11 flow (Default Chat + Meeting Bot + Camera Analysis + Agent Run + Project Chat \
    ...
    fn reseed_is_idempotent_for_harness_and_agent() {
        ...
        let flow_count: i64 = conn
            .query_row("SELECT COUNT(*) FROM flows", [], |r| r.get(0))
            .unwrap();
        assert_eq!(flow_count, 11, "ponowny seed nie duplikuje flow");
    ...
    fn reseed_after_rename_does_not_collide() {
        ...
        let total: i64 = conn
            .query_row("SELECT COUNT(*) FROM flows", [], |r| r.get(0))
            .unwrap();
        assert_eq!(total, 11, "rename nie moze tworzyc drugiego flow");
    ...
    fn factory_flows_compile_streaming_and_seed_respects_user_edits() {
        ...
        assert!(!super::is_factory_flow(super::PS_CHAT_FLOW_ID));
