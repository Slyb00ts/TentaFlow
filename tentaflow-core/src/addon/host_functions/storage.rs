...
pub fn storage_get(
    ...
    if key.len() > MAX_KEY_LENGTH {
        audit_log(caller.data(), "storage.get", Some("storage"), Some(&key[..64]), "error", Some("klucz za dlugi (max 1024B)"));
        return ABI_ERR_OPERATION;
    }

    // Sprawdz uprawnienie storage (ro)
    if !check_permission(caller.data(), "storage", None) {
        audit_log(caller.data(), "storage.get", Some("storage"), Some(&key), "denied", None);
    ...
    match value {
        Some(data) => {
            audit_log(caller.data(), "storage.get", Some("storage"), Some(&key), "ok", None);
            write_guest_output(&memory, &mut caller, out_ptr, out_cap, out_len_ptr, &data)
        }
        None => {
            audit_log(caller.data(), "storage.get", Some("storage"), Some(&key), "ok", Some("not found"));
            ABI_ERR_NOT_FOUND
        }
    }
}
...
pub fn storage_set(
    ...
    // CR-009: Walidacja dlugosci klucza
    if key.len() > MAX_KEY_LENGTH {
        audit_log(caller.data(), "storage.set", Some("storage"), Some(&key[..64]), "error", Some("klucz za dlugi (max 1024B)"));
        return ABI_ERR_OPERATION;
    }
    ...
    if value.len() > MAX_VALUE_SIZE {
        audit_log(caller.data(), "storage.set", Some("storage"), Some(&key), "error", Some("wartosc za duza (max 1MB)"));
        return ABI_ERR_OPERATION;
    }

    // Sprawdz uprawnienie storage (rw)
    if !check_permission(caller.data(), "storage", None) {
        audit_log(caller.data(), "storage.set", Some("storage"), Some(&key), "denied", None);
        return ABI_ERR_PERMISSION;
    }
    ...
    let within_limit = {
        match caller.data().db.lock() {
            Ok(conn) => {
                ...
                // Pobierz limit
                let limit_mb: i64 = conn.query_row(
    ...
    if !within_limit {
        audit_log(caller.data(), "storage.set", Some("storage"), Some(&key), "error", Some("storage limit exceeded"));
        return ABI_ERR_OPERATION;
    }
    ...
    if !key_count_ok {
        audit_log(caller.data(), "storage.set", Some("storage"), Some(&key), "error",
            Some(&format!("limit kluczy przekroczony (max {})", MAX_KEYS_PER_ADDON)));
        return ABI_ERR_OPERATION;
    }
    ...
    match result {
        Ok(_) => {
            audit_log(caller.data(), "storage.set", Some("storage"), Some(&key), "ok", None);
            ABI_OK
        }
        Err(e) => {
            let msg = e.to_string();
            audit_log(caller.data(), "storage.set", Some("storage"), Some(&key), "error", Some(&msg));
            ABI_ERR_OPERATION
        }
    }
}
...
pub fn storage_delete(
    ...
    if !check_permission(caller.data(), "storage", None) {
        audit_log(caller.data(), "storage.delete", Some("storage"), Some(&key), "denied", None);
        return ABI_ERR_PERMISSION;
    }
    ...
    match result {
        Ok(_) => {
            audit_log(caller.data(), "storage.delete", Some("storage"), Some(&key), "ok", None);
            ABI_OK
        }
        Err(e) => {
            let msg = e.to_string();
            audit_log(caller.data(), "storage.delete", Some("storage"), Some(&key), "error", Some(&msg));
            ABI_ERR_OPERATION
        }
    }
}
...
pub fn storage_list(
    ...
    let prefix = if prefix_ptr != 0 && prefix_len > 0 {
        read_guest_string(&memory, &caller, prefix_ptr, prefix_len)
            .map(|s| s.to_string())
    } else {
        None
    };
    ...
    // Sprawdz uprawnienie storage (ro)
    if !check_permission(caller.data(), "storage", None) {
        audit_log(caller.data(), "storage.list", Some("storage"), prefix.as_deref(), "denied", None);
        return ABI_ERR_PERMISSION;
    }
    ...
    let keys: Vec<String> = {
        ...
        let result = if let Some(ref pfx) = prefix {
            ...
            let rows: Vec<String> = match stmt.query_map(
                rusqlite::params![&addon_id, &instance_id],
                |row| row.get::<_, String>(0),
            ) {
                Ok(r) => r.filter_map(|r| r.ok()).collect(),
                Err(_) => return ABI_ERR_OPERATION,
    ...
    audit_log(caller.data(), "storage.list", Some("storage"), prefix.as_deref(), "ok", None);
