...
pub fn dispatch_invoke(
    ...
    match run_invoke(scheduler, &state.addon_id, &input.flow_id, input.input.clone(), input.wait_ms) {
        Ok(out) => {
            audit(
                state,
                ACTION_INVOKE,
...
pub fn run_invoke(
    ...
    let res = block_on_runtime(scheduler.invoke(addon_id, flow_id, input, wait_ms));
    match res {
