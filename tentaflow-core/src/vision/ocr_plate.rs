...
impl PlateOcr {
    ...
    pub fn read_batch(
        ...
        let mut dump_grays: Vec<Vec<u8>> = if dump {
            Vec::with_capacity(n)
        } else {
            Vec::new()
        };
        let mut dump_deskews: Vec<Option<(Vec<u8>, u32, u32)>> = if dump {
            Vec::with_capacity(n)
        } else {
            Vec::new()
        };
        ...
        let per_item: Vec<Vec<f32>> = self.pool.run(move |session| {
            ...
            if logits.len() < n * expected {
                bail!(
                    "ocr_plate: batch logits len {} < {n}*{expected}",
                    logits.len()
                );
            }
            Ok((0..n)
...
mod plate_format_tests {
    ...
    fn rejects_garbage_reads_from_occluded_plates() {
        ...
        assert!(
            !waliduj_tablice_pl("M88901"),
            "1 leading letter is not a PL plate"
        );
        ...
        assert!(
            waliduj_tablice_pl("NZ2476"),
            "NZ = valid voivodeship code, 6 chars"
        );
    }
    ...
    fn accepts_real_plates_rejects_bad_prefix_and_shapes() {
        ...
        assert!(
            !waliduj_tablice_pl("AB12345"),
            "A is not a voivodeship first letter"
        );
