...
impl VehicleDetector {
    ...
    pub fn load() -> Result<Self> {
        ...
        .map_err(|e| anyhow!("building YOLOv8 vehicle ort session pool of {n} session(s): {e:#}"))?;
        info!(
    ...
    pub fn detect_device_pixels(&self, frames: &[(&[u8], u32, u32)]) -> Result<Vec<Vec<Detection>>> {
        if frames.is_empty() {
            return Ok(Vec::new());
        }
        let res = RESOLUTION as usize;
    ...
    fn forward_device_ptr(&self, dev_ptr: usize, n: usize, res: usize) -> Result<Vec<Vec<Detection>>> {
        let (out_owned, attrs, anchors) = self.pool.run(move |session| {
...
pub(crate) fn fill_frame_yolo(data: &mut [f32], bi: usize, rgb: &[u8], w: u32, h: u32) -> Result<()> {
    let res = RESOLUTION as usize;
    let plane = res * res;
    let base = bi * 3 * plane;
