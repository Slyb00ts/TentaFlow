...
fn main() {
    ...
    // Pobierz (jezeli brak) modele audio embedded w binarce: silero VAD i
    // WeSpeaker diarization. include_bytes! w runtime, ekstrakcja do
    // data_local_dir przy pierwszym uruchomieniu. Bez tego user-installer
    // musialby pamietac o setup.sh.
    embed_audio_models(&out_dir_env);

    // Embeduj 5 modeli vision z `tentaflow-core/models/vision/` (pobrane przez
    // `scripts/setup.sh::download_vision_models`). Build.rs sam nie pobiera —
    // zeby cargo build offline tez mial deterministyczne zachowanie. Brak
    // pliku → empty placeholder (jak audio), runtime extractor zwraca None
    // i deploy zglasza "model nie zaladowany".
    embed_vision_models(&out_dir_env);
...
// =============================================================================
// embed_audio_models — pobiera silero_vad.onnx + WeSpeaker embedding.onnx,
// generuje audio_models_embed.rs z `include_bytes!` na obu plikach. Runtime
// ekstrahuje do `~/Library/Application Support/tentaflow/models/` przy
// pierwszym uruchomieniu (helper w `crate::diarization::extract_models`).
//
// Bez tego user dostajacy zbudowana binarke `tentaflow` musialby pamietac
// o setup.sh i uruchamianiu z konkretnego CWD. Po embed: zero zewnetrznych
// zaleznosci.
// =============================================================================

fn embed_audio_models(out_dir: &Path) {
    let dest = out_dir.join("audio_models");
    std::fs::create_dir_all(&dest).ok();

    let silero = dest.join("silero_vad.onnx");
    let silero_url =
        "https://github.com/snakers4/silero-vad/raw/v5.1/src/silero_vad/data/silero_vad.onnx";
    if !silero.exists() {
        if let Err(e) = download_file(silero_url, &silero) {
            println!(
                "cargo:warning=embed_audio_models: nie udalo sie pobrac silero_vad.onnx: {} — pusty placeholder",
                e
            );
            std::fs::write(&silero, b"").ok();
        } else {
            println!(
                "cargo:warning=silero_vad.onnx pobrany ({} KB)",
                std::fs::metadata(&silero).map(|m| m.len() / 1024).unwrap_or(0)
            );
        }
    }

    let embedding = dest.join("embedding.onnx");
    // ECAPA-TDNN 512 LM — loader `tentaflow-voice/src/wespeaker.rs` ma
    // hardcodowane nazwy tensorow tej architektury (model.layer1.conv +
    // model.layer{2,3,4}.se_res2block). Wczesniej URL wskazywal na
    // wespeaker-voxceleb-resnet34-LM (czysty ResNet34, layout `model.seg_1.*`)
    // i loader rzucal "Brak tensora 'model.layer1.conv.weight'" — diarization
    // wylaczona.
    let embedding_url = "https://huggingface.co/Wespeaker/wespeaker-ecapa-tdnn512-LM/resolve/main/voxceleb_ECAPA512_LM.onnx";
    if !embedding.exists() {
        if let Err(e) = download_file(embedding_url, &embedding) {
            println!(
                "cargo:warning=embed_audio_models: nie udalo sie pobrac embedding.onnx: {} — pusty placeholder",
                e
            );
            std::fs::write(&embedding, b"").ok();
        } else {
            println!(
                "cargo:warning=embedding.onnx pobrany ({} KB)",
                std::fs::metadata(&embedding).map(|m| m.len() / 1024).unwrap_or(0)
            );
        }
    }

    // audio_models_embed.rs — referuje pliki z OUT_DIR
    let rs = format!(
        r#"// AUTO-GENERATED by build.rs::embed_audio_models — DO NOT EDIT.
pub const SILERO_VAD_ONNX: &[u8] = include_bytes!(r"{silero}");
pub const WESPEAKER_EMBEDDING_ONNX: &[u8] = include_bytes!(r"{embedding}");
"#,
        silero = silero.display(),
        embedding = embedding.display(),
    );
    std::fs::write(out_dir.join("audio_models_embed.rs"), rs).ok();
}
...
//
// Lista modeli (klucz = stale Rust uzywane w `vision_models.rs`):
//   - YOLOV8_FACE_ONNX           — yolov8-face.onnx (faktycznie YOLOv11n-face)
//   - SCRFD_ONNX                 — scrfd.onnx
//   - MIVOLO_AGE_ONNX            — mivolo_age.onnx (placeholder GoogLeNet)
...
fn embed_vision_models(out_dir: &Path) {
    ...
    let src_dir = manifest_dir.join("models").join("vision");

    // Lista (const_name, source_filename). Source pliki mieszkaja w
    // `tentaflow-core/models/vision/`. Build.rs kopiuje je do OUT_DIR zeby
    // `include_bytes!` w wygenerowanym .rs nie zalezal od scieżki repo
    // (kopia `cargo publish`-able).
    let entries: &[(&str, &str)] = &[
        ("YOLOV8_FACE_ONNX", "yolov8-face.onnx"),
        ("SCRFD_ONNX", "scrfd.onnx"),
        ("MIVOLO_AGE_ONNX", "mivolo_age.onnx"),
    ...
    let mut rs = String::from(
        "// AUTO-GENERATED by build.rs::embed_vision_models — DO NOT EDIT.\n",
    );

    for (const_name, fname) in entries {
        let src = src_dir.join(fname);
        let dst = dest_dir.join(fname);
        // Cargo rerun-if-changed sledzi obecnosc pliku zrodlowego — gdy
        // user odpali setup.sh i pobierze brakujacy model, kolejny build
        // automatycznie go wciagnie.
        println!("cargo:rerun-if-changed={}", src.display());

        let copied_bytes: usize = match std::fs::read(&src) {
            Ok(bytes) if !bytes.is_empty() => {
                let len = bytes.len();
                std::fs::write(&dst, &bytes).ok();
                println!(
                    "cargo:warning=vision: {} embedowany ({} KB) z {}",
                    fname,
                    len / 1024,
                    src.display()
                );
            ...
            _ => {
                // Brak pliku albo pusty — placeholder. Konsystentne z
                // patternem audio_models::extract_blob.
                std::fs::write(&dst, b"").ok();
                println!(
                    "cargo:warning=vision: {} brak — pusty placeholder (uzyj setup.sh albo dostarcz {})",
                    fname,
                    src.display()
                );
                0
        ...
        rs.push_str(&format!(
            "pub const {name}: &[u8] = include_bytes!(r\"{path}\");\n",
            name = const_name,
            path = dst.display(),
        ));
    }

    std::fs::write(out_dir.join("vision_models_embed.rs"), rs).ok();
}

fn download_file(url: &str, dest: &Path) -> Result<(), String> {
    // Stary good `curl -fL`. wget jako fallback. Bez external crate'ow w
    // build.rs zeby nie blokowac CI offline-only buildow przez network deps.
    if Command::new("curl")
        .args(["-fL", "-o"])
        .arg(dest)
        .arg(url)
    ...
    Err(format!("ani curl ani wget nie pobraly {}", url))
}
