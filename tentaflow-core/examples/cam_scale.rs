...
fn main() -> anyhow::Result<()> {
    ...
    #[cfg(feature = "inference-supertonic")]
    let state_batcher = Arc::new(InferenceBatcher::<Vec<String>>::new(
        MAX_BATCH,
        batch_window(),
        {
            ...
            Arc::new(move |crops: &[(Arc<[u8]>, u32, u32)]| {
                let refs: Vec<(&[u8], u32, u32)> = crops
                    .iter()
                    .map(|(cr, w, h)| (cr.as_ref(), *w, *h))
                    .collect();
    ...
    #[cfg(feature = "inference-supertonic")]
    let plate_batcher = Arc::new(InferenceBatcher::<(Option<String>, f32)>::new(
        MAX_BATCH,
        batch_window(),
        {
            let o = ocr.clone();
    ...
    let state_crops: Vec<(Arc<[u8]>, u32, u32)> = (0..args.dets)
        .map(|_| {
            (
                Arc::from(vec![128u8; 96 * 96 * 3].into_boxed_slice()),
                96u32,
                96u32,
            )
        })
        .collect();
    let plate_crops: Vec<(Arc<[u8]>, u32, u32)> = vec![(
        Arc::from(vec![128u8; 48 * 120 * 3].into_boxed_slice()),
        120u32,
        48u32,
    )];
            ...
            workers.push(std::thread::spawn(move || {
                ...
                while !stop.load(Ordering::Relaxed) {
                    ...
                    let did_detect_batched = if batched {
                        let _ =
                            detect_batcher.submit(det_frame_arc.clone(), RESOLUTION, RESOLUTION);
                        true
                    } else {
                        false
                    };
                    ...
                    if !did_detect_batched {
                        let _ = detector
                            .detect_batch(&[(det_frame.as_slice(), RESOLUTION, RESOLUTION)], None);
                    }
                    if !detect_only {
