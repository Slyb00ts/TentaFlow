...
fn write_wav(samples: &[f32], sr: u32, path: &str) -> Result<()> {
    ...
    b.extend(b"RIFF");
    b.extend(&((36 + ds) as u32).to_le_bytes());
    b.extend(b"WAVE");
    b.extend(b"fmt ");
    ...
    b.extend(&(sr * bps as u32).to_le_bytes());
    b.extend(&(bps as u16).to_le_bytes());
    b.extend(&16u16.to_le_bytes());
    b.extend(b"data");
    b.extend(&(ds as u32).to_le_bytes());
    for s in samples {
...
async fn main() -> Result<()> {
    ...
    let r = apple.synthesize(SynthesizeParams {
        text: "Test integracji silnikow".into(),
        speaker_id: 0,
        speed: 1.0,
    })?;
    ...
    k.load_model(&kp)?;
    let r = k.synthesize(SynthesizeParams {
        text: "Both engines work in the same Rust binary.".into(),
        speaker_id: 0,
        speed: 1.0,
    })?;
