...
fn main() -> anyhow::Result<()> {
    ...
    let crop = |b: &[f32;4]| -> Option<(Vec<u8>,u32,u32)> {
        let x0=(b[0]*w as f32).round().max(0.0) as u32; let y0=(b[1]*h as f32).round().max(0.0) as u32;
        let cw=((b[2]*w as f32).round() as u32).min(w.saturating_sub(x0)); let ch=((b[3]*h as f32).round() as u32).min(h.saturating_sub(y0));
        if cw<8||ch<8 {return None;}
        let mut c=Vec::with_capacity((cw*ch*3)as usize);
        for yy in y0..y0+ch { let o=((yy*w+x0)*3)as usize; c.extend_from_slice(&rgb[o..o+(cw*3)as usize]); }
        Some((c,cw,ch))
    };
    let dets = det.detect(rgb, w, h)?;
    println!("{}  {} detections:", path.rsplit('/').next().unwrap(), dets.len());
    for d in &dets {
        let mut extra=String::new();
        if d.klasa.starts_with("nalepka")||d.klasa=="znak_srodowiskowy"||d.klasa=="termometr" {
            if let Some((c,cw,ch))=crop(&d.bbox){ extra=format!("stan={:?}", stan.classify(&c,cw,ch)?); }
        } else if d.klasa=="tablica_rejestracyjna" {
            if let Some((c,cw,ch))=crop(&d.bbox){ extra=format!("plate={:?}", ocr.read(&c,cw,ch)?); }
        }
