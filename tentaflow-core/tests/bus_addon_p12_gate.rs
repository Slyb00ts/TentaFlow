...
async fn run_gate(cycles: u64) {
...

    {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            .expect("bus::init");
            // `bus::global()` was removed on main (it only ever resolved the
            // single-instance shim); this test still called it, so main does not
            // build its own integration tests. One running instance is what the
            // fixture just started.
            let svc = bus::running_instances()
                .into_iter()
                .next()
