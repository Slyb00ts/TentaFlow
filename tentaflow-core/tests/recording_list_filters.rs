...
fn filters_compose_with_and() {
    let (_d, pool) = open_pool();
    insert_event(&pool, "clip_hit", "cam-1", Some("WGM12345"), Some("30/1202"), 5_000);
    // Same plate, out of the date window → excluded by the AND.
    insert_event(&pool, "clip_late", "cam-1", Some("WGM12345"), Some("30/1202"), 9_999);
    // In window, wrong plate → excluded.
    insert_event(&pool, "clip_wrong", "cam-1", Some("KR9021X"), Some("30/1202"), 5_000);
...
fn insert_populates_search_columns_and_thumbs() {
    let (_d, pool) = open_pool();
    insert_event(&pool, "clip_a", "cam-1", Some("WGM12345"), Some("30/1202"), 1_000);
    let filters = RecordingListFilters {
        kind: Some("segment"),
        ..Default::default()
    };
