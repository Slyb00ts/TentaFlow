...
use tentaflow_core::addon::runtime::{
    create_engine, compile_module, create_linker, instantiate,
};
...
use tentaflow_core::addon::{AddonState, AddonManifest};
...
const TEST_ADDON_WASM: &str = "addons/test-addon/target/wasm32-unknown-unknown/release/tentaflow_addon_test.wasm";
...
fn create_test_db() -> db::DbPool {
    let conn = rusqlite::Connection::open_in_memory()
        .expect("Nie udalo sie otworzyc in-memory DB");
    ...
    db::migrations::run(&conn)
        .expect("Blad uruchamiania migracji");
...
fn create_wasm_instance(
    ...
    host_functions::register_host_functions(&mut linker)
        .expect("Blad rejestracji host functions");

    let instance = instantiate(&linker, &mut store, &module)
        .expect("Blad instancjacji WASM");
...
fn call_on_request(
    ...
    let input_ptr = alloc_fn.call(&mut *store, request_bytes.len() as i32)
    ...
    let out_ptr = alloc_fn.call(&mut *store, out_cap)
        .map_err(|e| format!("alloc output blad: {}", e))?;

    // Alloc miejsce na dlugosc wyniku (4 bajty)
    let out_len_ptr = alloc_fn.call(&mut *store, 4)
    ...
    let out_len = i32::from_le_bytes([
        out_len_bytes[0], out_len_bytes[1], out_len_bytes[2], out_len_bytes[3]
    ]);

    // Odczytaj odpowiedz JSON
    let result_bytes = &mem_data[out_ptr as usize..out_ptr as usize + out_len as usize];
    serde_json::from_slice(result_bytes)
        .map_err(|e| format!("Blad parsowania odpowiedzi: {}", e))
}
...
fn addon_wasm_loads() {
    ...
    let required_exports = ["on_install", "on_start", "on_stop", "on_request", "alloc", "dealloc", "memory"];
...
fn addon_on_start_on_stop() {
    ...
    let on_stop = instance
        ...
        .expect("Brak on_stop");
    let result = on_stop.call(&mut store, ())
        .expect("on_stop trap");
...
fn addon_tool_storage() {
    ...
    assert_eq!(response["ok"], true, "test_storage powinno zwrocic ok=true: {:?}", response);
    assert_eq!(
        response["data"]["match"], true,
        "Odczytana wartosc powinna byc rowna zapisanej: {:?}", response
    );
    assert_eq!(
...
fn addon_fuel_metering() {
    ...
    let memory = instance.get_memory(&mut store, "memory").expect("Brak memory");
    memory.data_mut(&mut store)[input_ptr as usize..input_ptr as usize + request_bytes.len()]
        .copy_from_slice(&request_bytes);
...
fn addon_lifecycle_full() {
    ...
    std::fs::copy(&wasm_src, &wasm_dst)
        .unwrap_or_else(|e| panic!("Nie udalo sie skopiowac WASM: {:?} -> {:?}: {}", wasm_src, wasm_dst, e));
    ...
    // 1. Instalacja addonu
    let manifest = lifecycle::install(&addon_dir, &db)
        .expect("Blad instalacji addonu");

    assert_eq!(manifest.addon_id, "test-addon");
    assert_eq!(manifest.version, "1.0.0");
    assert!(!manifest.tools.is_empty(), "Addon powinien miec zarejestrowane narzedzia");
    ...
    host_functions::register_host_functions(&mut linker)
        .expect("Blad rejestracji host functions");
    let instance = instantiate(&linker, &mut store, &module)
        .expect("Blad instancjacji");

    // 5. on_start
    let on_start = instance
        .get_typed_func::<(), i32>(&mut store, "on_start")
        .expect("Brak on_start");
    let start_result = on_start.call(&mut store, ())
        .expect("on_start trap");
    ...
    let on_stop = instance
        .get_typed_func::<(), i32>(&mut store, "on_stop")
        .expect("Brak on_stop");
    let stop_result = on_stop.call(&mut store, ())
        .expect("on_stop trap");
    assert_eq!(stop_result, 0, "on_stop powinno zwrocic 0");

    // 8. Odinstaluj addon
    lifecycle::uninstall("test-addon", &db)
        .expect("Blad deinstalacji");
    ...
    // 9. Sprawdz cleanup w DB
    {
        ...
        assert!(!exists, "Addon powinien zostac usuniety z DB po deinstalacji");
    }
}
...
fn addon_unknown_tool() {
    ...
    assert_eq!(
        response["ok"], false,
        "Nieznane narzedzie powinno zwrocic ok=false: {:?}", response
    );
}
...
fn addon_manifest_parsing() {
    ...
    let _content = std::fs::read_to_string(&manifest_path)
        .expect("Nie udalo sie odczytac manifest.toml");
    ...
    if needs_copy {
        std::fs::copy(&wasm_src, &wasm_dst)
            .expect("Nie udalo sie skopiowac WASM");
    }
    ...
    let db = create_test_db();
    let manifest = lifecycle::install(&addon_dir, &db)
        .expect("Blad parsowania manifest.toml");
    ...
    // Sprawdz uprawnienia
    assert!(!manifest.permissions.is_empty(), "Powinny byc uprawnienia");
    let perm_types: Vec<&str> = manifest.permissions.iter()
        .map(|p| p.permission_type.as_str())
        .collect();
    assert!(perm_types.contains(&"storage"), "Brak uprawnienia 'storage'");
    assert!(perm_types.contains(&"log"), "Brak uprawnienia 'log'");

    // Sprawdz narzedzia
    let tool_names: Vec<&str> = manifest.tools.iter()
        .map(|t| t.name.as_str())
        .collect();
    assert!(tool_names.contains(&"echo"), "Brak narzedzia 'echo'");
    assert!(tool_names.contains(&"test_storage"), "Brak narzedzia 'test_storage'");
    assert!(tool_names.contains(&"test_crash"), "Brak narzedzia 'test_crash'");

    // Sprawdz declared_permissions
    assert_eq!(manifest.declared_permissions.len(), 2, "Powinny byc 2 deklarowane uprawnienia");
    let decl_ids: Vec<&str> = manifest.declared_permissions.iter()
        .map(|p| p.id.as_str())
        .collect();
    assert!(decl_ids.contains(&"test_read"));
    assert!(decl_ids.contains(&"test_write"));
}
...
const MALICIOUS_ADDON_WASM: &str = "addons/malicious-addon/target/wasm32-unknown-unknown/release/tentaflow_addon_malicious.wasm";
...
fn create_malicious_instance(
    ...
    let state = create_addon_state_with_id(
        db,
        permissions,
        "malicious-addon",
        "malicious-instance-001",
    );
    let mut store = create_test_store(&engine, state, fuel);
...
fn malicious_storage_isolation() {
    ...
    // 1. Zainstaluj test-addon i zapisz dane w storage
    {
        ...
        assert_eq!(response["ok"], true, "test-addon powinien zapisac dane: {:?}", response);
    }
    ...
    assert!(
        ...
        "Nie powinno byc podatnosci: {:?}", response
    );
}
...
fn malicious_unauthorized_http() {
    ...
    assert_eq!(
        response["ok"], true,
        "HTTP bez uprawnienia powinno byc zablokowane: {:?}", response
    );
    assert!(
        response.get("vulnerability").is_none(),
        "Nie powinno byc podatnosci HTTP: {:?}", response
    );
}
...
fn malicious_unauthorized_secrets() {
    ...
    assert_eq!(
        response["ok"], true,
        "Secrets bez uprawnienia powinno byc zablokowane: {:?}", response
    );
    assert!(
        response.get("vulnerability").is_none(),
        "Nie powinno byc podatnosci secrets: {:?}", response
    );
}
...
fn malicious_unauthorized_network() {
    ...
    assert_eq!(
        response["ok"], true,
        "Network bez uprawnienia powinno byc zablokowane: {:?}", response
    );
    assert!(
        response.get("vulnerability").is_none(),
        "Nie powinno byc podatnosci network: {:?}", response
    );
}
...
fn malicious_memory_escape() {
    ...
    let err = call_on_request_expect_trap(
        &mut store,
        &instance,
        "try_memory_escape",
    );
...
fn malicious_stack_overflow() {
    ...
    let input_ptr = match alloc_fn.call(&mut store, request_bytes.len() as i32) {
        ...
        Err(e) => {
            ...
            assert!(err.contains("fuel") || err.contains("trap"), "Oczekiwano fuel/trap: {}", err);
            return;
        }
    };
...
fn malicious_cross_addon_storage() {
    ...
    assert_eq!(
        response["ok"], true,
        "Cross-addon storage powinno byc izolowane: {:?}", response
    );
    assert!(
        response.get("vulnerability").is_none(),
        "Nie powinno byc podatnosci cross-addon storage: {:?}", response
    );

    // 3. Sprawdz ze dane teams nadal sa w DB (nie zostaly uszkodzone)
    {
        let conn = db.lock().unwrap();
        let teams_data: String = conn.query_row(
...
fn malicious_bad_pointers() {
    ...
    assert_eq!(
        response["ok"], true,
        "Bounds check powinien dzialac: {:?}", response
    );
    assert!(
        response.get("vulnerability").is_none(),
        "Nie powinno byc podatnosci bounds: {:?}", response
    );
...
fn malicious_direct_tcp() {
    ...
    assert!(
        err_msg.contains("indirect call")
            ...
            || err_msg.contains("error while executing"),
        "Oczekiwano trap od call_indirect, dostano: {}", err
    );
}
...
fn malicious_write_file() {
    ...
    assert!(
        err_msg.contains("out of bounds")
            ...
            || err_msg.contains("memory")
            || err_msg.contains("error while executing"),
        "Oczekiwano trap od out-of-bounds write, dostano: {}", err
    );
}
...
fn malicious_read_file() {
    ...
    match result {
        Ok(response) => {
            ...
            assert!(
                response.get("vulnerability").is_none(),
                "Odczyt plikow hosta nie powinien byc mozliwy w WASM: {:?}", response
            );
        }
        Err(e) => {
            ...
            assert!(
                err_msg.contains("out of bounds")
                    ...
                    || err_msg.contains("memory")
                    || err_msg.contains("error while executing"),
                "Oczekiwano trap od out-of-bounds read, dostano: {}", e
            );
        }
    }
}
...
fn malicious_read_env() {
    ...
    assert_eq!(
        response["ok"], true,
        "Pamiec WASM nie powinna zawierac zmiennych srodowiskowych hosta: {:?}", response
    );
    assert!(
        response.get("vulnerability").is_none(),
        "Nie powinno byc podatnosci env: {:?}", response
    );
}
