...
//       Orlen vs image::imageops::resize (Triangle). Mierzy downscale do
...
/// Katalog z realnymi zdjeciami Orlen (PoC AI). Jesli niedostepny (CI bez
/// montowanego dysku), bench loguje i konczy bez panic.
const ORLEN_DIR: &str = "/mnt/abyss/Files/Dokumenty/Praca/Euvic/Tematy/Orlen/PoC AI/Zdjęcia/5";
...
fn bench_resize(c: &mut Criterion) {
    ...
    if images.is_empty() {
        eprintln!(
            "resize_perf: brak zdjec Orlen w {ORLEN_DIR} — bench pominiety (zamontuj dysk)"
        );
        return;
    }
