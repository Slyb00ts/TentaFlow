...
const S='/home/critix/repos/rust/TentaFlow/.pw-tmp'; const ORLEN='75303e4d-3583-423f-8aea-eff89a20eca3'; const sleep=(ms)=>new Promise(r=>setTimeout(r,ms));
...
try{
  ...
  await page.evaluate(()=>{const i=document.getElementById('ak-name'); if(i){i.value='remote-importtest'; i.dispatchEvent(new Event('input',{bubbles:true}));}}); await sleep(300);
  ...
  if(!scope.orlenPresent){ console.log('BLAD: brak wiersza ml_studio_export dla Orlen'); await page.screenshot({path:S+'/a-scope.png',fullPage:true}); }
    ...
    if(token){ fs.writeFileSync(S+'/target-key.txt', token); console.log('KLUCZ UTWORZONY, dlugosc:', token.length, 'prefiks:', token.slice(0,8)); }
