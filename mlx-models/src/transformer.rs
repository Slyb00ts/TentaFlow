...
pub struct ModelArgs {
    ...
    // Bias flags — some models (e.g. Bielik) use attention/MLP bias
    // even though their model_type is "llama".
    #[serde(default)]
...
impl ModelArgs {
    ...
    ///
    /// Uses the explicit `attention_bias` config field if set,
    /// otherwise falls back to model_type heuristic (Qwen2/Qwen3 use bias).
    pub fn qkv_bias(&self) -> bool {
        self.attention_bias || matches!(self.model_type.as_str(), "qwen2" | "qwen3")
    }

    /// Whether MLP projections should have bias.
...
impl Mlp {
    pub fn new(dim: i32, hidden_dim: i32) -> Result<Self, Exception> {
        Self::with_bias(dim, hidden_dim, false)
    }

    pub fn with_bias(dim: i32, hidden_dim: i32, bias: bool) -> Result<Self, Exception> {
...
impl TransformerBlock {
    pub fn new(args: &ModelArgs) -> Result<Self, Exception> {
        Ok(Self {
            ...
            mlp: Mlp::with_bias(args.hidden_size, args.intermediate_size, args.mlp_bias())?,
...
pub fn load_model(model_dir: impl AsRef<Path>) -> Result<Model, ModelError> {
    ...
    // Workaround: mlx-rs 0.25 QuantizedEmbedding nie ma #[param] na scales/biases,
    // wiec load_quantized_safetensors_weights nie moze ich zaladowac.
    // Recznie ladujemy brakujace klucze z safetensors.
    if quantization.is_some() {
        if let MaybeQuantized::Quantized(ref mut q_embed) = model.model.embed_tokens {
            ...
            for file_path in &safetensors_files {
                ...
                // Szukamy kluczy embed_tokens — moga miec prefix "model."
                for (key, value) in &loaded {
                    if key == "model.embed_tokens.scales" || key == "embed_tokens.scales" {
                        *q_embed.scales = value.clone();
                        tracing::info!("Zaladowano embed_tokens.scales (workaround)");
                    }
                    if key == "model.embed_tokens.biases" || key == "embed_tokens.biases" {
                        *q_embed.biases = value.clone();
                        tracing::info!("Zaladowano embed_tokens.biases (workaround)");
                    }
                    if key == "model.embed_tokens.weight" || key == "embed_tokens.weight" {
                        q_embed.inner.weight = mlx_rs::module::Param::new(value.clone());
                        tracing::info!("Zaladowano embed_tokens.weight (workaround)");
                    }
                }
            }
        }
