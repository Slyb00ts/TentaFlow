...
pub fn load_quantized_safetensors_weights<M: ModuleParametersExt>(
    ...
    if !skipped.is_empty() {
        tracing::warn!(
            ...
            "Weight loading: {} keys matched, {} keys skipped",
            matched, skipped.len()
        );
        for key in &skipped {
            tracing::warn!(key = %key, "Skipped weight key (no matching parameter)");
        }
    } else {
        tracing::info!(matched = matched, "All {} weight keys matched successfully", matched);
    }
...
fn remap_quantized_key(key: &str) -> Option<String> {
    if let Some(prefix) = key.strip_suffix(".weight") {
        ...
        // Kwantyzowane warstwy (QuantizedLinear/QuantizedEmbedding) przechowuja scales
        Some(format!("{prefix}.inner.scales"))
    } else if let Some(prefix) = key.strip_suffix(".biases") {
        // Kwantyzowane warstwy przechowuja biases (z "s" — nie mylić z .bias)
