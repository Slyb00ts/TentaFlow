...
def _expand_spans(table):
    """
    Expand colspan/rowspan in a BeautifulSoup <table> into a regular grid.
    Returns a list-of-lists of cell text strings.
    """
    rows = table.find_all('tr')
    if not rows:
        return []
    ...
    grid = [[None] * num_cols for _ in range(len(rows))]
        ...
        for cell in cells:
            ...
            for dr in range(rowspan):
                for dc in range(colspan):
                    rr, cc = r + dr, c + dc
                    if rr < len(grid) and cc < num_cols:
                        grid[rr][cc] = text
            c += colspan
...
def _grid_to_records(grid):
    """
    Convert a grid (list-of-lists) to a list of dicts using the first row as
    column headers. Duplicate or empty headers get disambiguated with a suffix.
    """
    if len(grid) < 2:
        return grid
...
def convert_html_table_to_json(html_content, records=True):
    """
    Convert HTML tables produced by ``latex_table_to_html`` into JSON.

    Parameters
    ----------
    html_content : str
        HTML string (may contain one or more ``<table>`` elements and
        surrounding non-table text).
    records : bool
        If *True* (default) the first row is treated as a header and each
        subsequent row becomes a ``{header: value, ...}`` dict.
        If *False* the table is returned as a plain list-of-lists.

    Returns
    -------
    str
        A JSON string.  If the input contains a single table the top-level
        value is a list (of dicts or lists).  If there are multiple tables
        the top-level value is a list of such lists.
    """
    soup = BeautifulSoup(html_content, 'html.parser')
    tables = soup.find_all('table')
    ...
    results = []
    for table in tables:
        grid = _expand_spans(table)
        if records:
            results.append(_grid_to_records(grid))
        else:
            results.append(grid)
...
def convert_html_table_to_csv(html_content):
    """
    Convert HTML tables produced by ``latex_table_to_html`` into CSV.

    Each table becomes a CSV block separated by a blank line.
    ...
    for table in tables:
        ...
        csv_blocks.append(buf.getvalue().rstrip('\r\n'))
