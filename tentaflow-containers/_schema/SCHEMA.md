...
| `provider` | enum | nie | Vendor grupujący serwis w zakładce katalogu: `tentaflow` albo `ibm`. Brak lub `tentaflow` = domyślna zakładka TentaFlow; `ibm` = dedykowana zakładka IBM. |
