# ===== File: llm/patches/dspark/patch_nvfp4_stages.py — nvfp4_ds_mla stage A/B/C patches =====
...
#
# The one thing upstream vLLM still does not carry: `nvfp4_ds_mla` is what makes
# DeepSeek V4's 1M context reachable on a Spark pair, and fp8 KV halves the pool.
import sys
from pathlib import Path


def _vllm_root() -> Path:
    """Locate the installed vllm package. Asking the interpreter keeps this
    working across a container (/usr/local/.../dist-packages), the older image
    layout (/opt/env/.../site-packages) and a native venv — a hardcoded path
    silently belongs to exactly one of them."""
    if len(sys.argv) > 1:
        p = Path(sys.argv[1])
