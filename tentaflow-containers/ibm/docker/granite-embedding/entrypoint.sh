...
# =============================================================================
# Plik: entrypoint.sh
# Opis: Uruchamia sidecar QUIC + vLLM w trybie embeddings (vllm --task embed)
#       rownolegle. Sidecar nasluchuje iroh natychmiast (klient tentaflow widzi
#       peera od razu); vLLM laduje model embeddingow w tle i serwuje
#       /v1/embeddings. Logi obu procesow trafiaja na stdout kontenera z
#       prefixem. PID 1 czeka na pierwszego z procesow ktory padnie - druga
#       konczymy grzecznie i exit z jego kodem (docker restart policy decyduje).
# =============================================================================
...
MODEL="${MODEL:?MODEL env required, np. 'ibm-granite/granite-embedding-278m-multilingual'}"
...
echo "[entrypoint] vllm serve $MODEL na 127.0.0.1:$VLLM_PORT (--task embed, ${#ENGINE_ARGS[@]} args)"
vllm serve "$MODEL" \
  --host 127.0.0.1 \
  --port "$VLLM_PORT" \
  --task embed \
