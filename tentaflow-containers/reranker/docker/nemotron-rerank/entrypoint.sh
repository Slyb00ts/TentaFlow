...
# =============================================================================
# Plik: entrypoint.sh
# Opis: Uruchamia sidecar QUIC + vLLM w trybie rerankingu (vllm serve
#       --task score) rownolegle. Sidecar nasluchuje iroh natychmiast (klient
#       tentaflow widzi peera od razu); vLLM laduje cross-encoder w tle i
#       serwuje /v1/rerank + /v1/score. Logi obu procesow trafiaja na stdout
#       kontenera z prefixem. PID 1 czeka na pierwszego z procesow ktory padnie
#       - druga konczymy grzecznie i exit z jego kodem (docker restart policy
#       decyduje co dalej).
# =============================================================================
...
MODEL="${MODEL:?MODEL env required, np. 'nvidia/llama-nemotron-rerank-1b-v2'}"
...
# --trust-remote-code bo architektura rerankera NVIDIA dostarcza wlasny kod
# modelu w repo HF.
echo "[entrypoint] vllm serve $MODEL na 127.0.0.1:$VLLM_PORT (--task score, ${#ENGINE_ARGS[@]} args)"
vllm serve "$MODEL" \
  --host 127.0.0.1 \
  --port "$VLLM_PORT" \
  --trust-remote-code \
