...
# =============================================================================
# Plik: entrypoint.sh
# Opis: Sidecar QUIC (5000/UDP) + uvicorn server (127.0.0.1:8087) startuja
...
echo "[entrypoint] start chatterbox server (uvicorn 127.0.0.1:$PORT)"
uvicorn server:app --host 127.0.0.1 --port "$PORT" 2>&1 \
  | sed -u 's/^/[chatterbox] /' &
ENGINE_PID=$!
echo "[entrypoint] chatterbox PID=$ENGINE_PID"
