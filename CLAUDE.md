...
- Dwa nowe adaptery Sync/Scrub i konsument UI są częścią tego samego nieukończonego przyrostu. Dwie nazwy w `dispatch/mod.rs` zarejestrowano po jawnej zgodzie użytkownika; Cargo, nowy WASM, pełna integracja UI i rzeczywisty odbiór Sync/Scrub z terminalnym JobGet oraz historią po restarcie pozostają bramkami. Historyczny pomiar CLI poniżej ich nie zastępuje.
