...
- `guest_writer_gate_probe.py` przygotowuje A0 dla service-mode movera: `prepare → local → global` na jednym UUID `04bb9cc8-63b2-4562-b6ef-46a5c0663733` i boot `56df71e4-c675-4961-bc64-a3dda64d6002`. Małe pliki na OS, bez mkfs dodatkowych dysków, transferu i unlink; prywatny root journal/lock, testowe branche dostępne od początku. Odróżnia lokalne mount RO od globalnego superblock RO przez dokładne aliasy, UID 1000/userns, held FD i reopen. Python VM 3.13.5 oraz `trackfd=False` pozwalają osobno sprawdzić mmap bez FD; ENODEV przy `cache.files=off` jest niedostępnością, nie dowodem blokowania. Direct backing write pozostaje dodatnią kontrolą zakresu FUSE. Runtime A0 jeszcze nieodebrany; to sonda kandydata, nie gotowa bramka produktu ani mover. Stare VM i pending nietknięte, bez retry/reset/cleanup.
