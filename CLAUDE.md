...
- `guest_writer_gate_probe.py` mierzy A0 dla service-mode movera (`prepare → local → global`), bez formatowania dodatkowych dysków, transferu i unlink. R3: VM `mOudMN`, UUID `5a6b69ca-df2a-4083-b90d-7bbd3f486a3f`, boot `95a783e6-9609-4d4b-a0cf-4725d87987f4`, dowody `crrYg8`; opublikowane źródło `3045178c…`, exact 214 PASS, preflight/prepare kod 0, local/global jeszcze niewykonane. `perMountBypass` i `umountOutcome=succeeded|busy|unexpected` są osobnymi faktami: busy nie oznacza odmontowania. Pełne mountinfo rodzica i aktorów, PID/start/namespace są utrwalane przed syscall; błędny odczyt lub zapis blokuje komendę. Globalne kryteria bez poluzowania. Prywatny root journal/lock i dostępne testowe branche; direct backing write jest kontrolą FUSE, nie izolacją branchy. Python 3.13+ / `trackfd=False`; ENODEV mmap to wariant nieprzetestowany. R1 odmówiło przed remount, r2 na `3dc2933f…` potwierdziło setup root→1000/caps 0/dumpability 0→1, ale local zakończyło rc 1/pending reopen: remount 0, umount EBUSY, aliasy nadal zapisują. Przyjęto tylko częściowy dowód per-mount RO ≠ globalna blokada; nie identyfikację źródła busy ani całe A0/E2-09. Stare VM bez retry/reset, global i izolacja branchy nadal otwarte. Szczegóły: `tests/infra/tentanas-vm/README.md`, sekcja sondy A0.
