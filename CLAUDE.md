...
- A1 `guest_branch_isolation_probe.py` / `test_guest_branch_isolation_probe.py`: jednorazowy dowód prywatnych branchy na tmpfs ≤8 MiB i publikacji tylko FUSE (VM `DpCJ1y`, UUID `77b5de74-031a-4d95-9e36-9d52b9e8ee96`, boot `c32a49c0-a9b1-46a6-94a5-bc96242b9af5`, dowody `ByvrXm`; run jeszcze niewykonany). Bez mkfs dodatkowych dysków, transferu/unlink i zmian produktu. Zamknięte cele UID 1000/userns, żywe PID/start/ns/FD, raw/pending przed ACK/syscall; global RO trzech aliasów i dodatnia kontrola root workera. Przypięte helpery A0 bez zmiany jego globals. Worker/aktorzy bounded, mergerfs pozostaje do odczytu PM; tmpfs volatile, crash/recovery i mover nieodebrane, brak retry/unmount/cleanup. Szczegóły i lokalne testy realnych procesów/SCM/journala w README infrastruktury.
