...
- `guest_cache_probe.py` to zamknięta sonda E2-03 dla jednego UUID `44fc2cf1-06f3-4135-a26b-220a2d5beba5`: preflight → nc → race → enospc, logiczny cache=data1 32 GiB/data=parity 40 GiB. Piny binarek i root-owned fixture, dokładne seriale/FS/mounty/opcje, trwały pending, ten sam boot oraz inode locka EX/NB odmawiają podmian i ponowień. Operator zapewnia 64 GiB wolnego hosta; sonda pilnuje 2 GiB OS i limitu fillera 32 GiB/20 minut. OS służy małemu journalowi/logom, trzy pozostałe puste dyski są nietykalne. Pomiary VM sondą jeszcze niewykonane; lokalne testy guardów/journalu/locka/pipe nie dowodzą FUSE ani produkcyjnego movera. NC wyklucza automatyczny spill; nierozstrzygnięty append bez errno nie staje się sukcesem relokacji. Brak retry/reset/cleanup; szczegóły w README stanowiska.
