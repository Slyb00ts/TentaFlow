...
- `guest_writer_gate_probe.py` przygotowuje A0 dla service-mode movera: `prepare → local → global` na nowej VM r2 `PM0Ymz`, UUID `b1fa1b6e-bd25-41c9-aa6f-a8f2140544aa`, boot `e26f6d28-942d-41e4-8c92-285c4d540d24`. Małe pliki na OS, bez mkfs dodatkowych dysków, transferu i unlink; prywatny root journal/lock, testowe branche dostępne od początku. Odróżnia lokalne mount RO od globalnego superblock RO przez dokładne aliasy, UID 1000/userns, held FD i reopen. Python VM 3.13.5 oraz `trackfd=False` pozwalają osobno sprawdzić mmap bez FD; ENODEV przy `cache.files=off` jest niedostępnością, nie dowodem blokowania. Direct backing write pozostaje dodatnią kontrolą zakresu FUSE. R1 `kaD6aK` odmówiło przed remount na `/proc/self/setgroups` (errno 13, potem wtórny KeyError), zachowując `prepare/pending=actor-start`; jego dumpability nie zmierzono. Nowy child potwierdza UID/caps przed namespace-only dumpability 1, wymusza RLIMIT_CORE 0 i utrwala setup/pierwotny fatal. Lokalny rzeczywisty userns test 0→EACCES i 1→mapowania nie dowodzi przejścia root→1000 na VM. R2 tylko przygotowane, A0 jeszcze nieodebrane; to sonda kandydata, nie gotowa bramka produktu ani mover. Stare VM i pending nietknięte, bez retry/reset/cleanup.
