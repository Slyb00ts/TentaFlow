...
- A2.1: helper 0.9 i `elastic_namespace.rs` obsługują nowe prywatne Create (journal schema 2); istniejące publiczne schema 1 bez migracji/adopcji. Kotwica wiąże boot/PID/start/namespace/binarkę przez `sha2` oraz FUSE. Child utrwala zamiar przed publikacją i kończy po ACK, parent wyłącznie odczytuje journal; wszystkie prywatne operacje sprawdzają namespace i rzeczywistą publikację hosta, bez fallbacku. Restore zachowuje FSUUID, bez mkfs; żywa kotwica może ponownie wyeksportować ten sam FUSE na pusty cel. PM odebrał robocze 163 testy helpera i 262 core, Clippy bez nowych diagnostyk względem A2.0 (nadal 101). Rzeczywiste odbiory lifecycle L i parity P pozostają niewykonane; nie jest to zamknięcie A2/E2-09 ani movera. Szczegóły w README infrastruktury, sekcja A2.1.
