...
- Odczyt helpera waliduje właściciela, intencję, role i tożsamość filesystemów niezależnie od historycznego etapu journala. Potwierdzony brak montowań może dać pending zamiast odrzucenia historycznego Ready; brak pomiaru pozostaje unknown. Wynik mutacji Ready nadal wymaga wszystkich obecnych i zamontowanych filesystemów, unii oraz checkpointu sync przy parity.
...
- `guest_writer_gate_probe.py` mierzy A0 dla service-mode movera: `prepare → local → global` na VM r2 `PM0Ymz`, UUID `b1fa1b6e-bd25-41c9-aa6f-a8f2140544aa`, boot `e26f6d28-942d-41e4-8c92-285c4d540d24`. Małe pliki na OS, bez mkfs dodatkowych dysków, transferu i unlink; prywatny root journal/lock, testowe branche dostępne od początku. Odróżnia lokalne mount RO od globalnego superblock RO przez dokładne aliasy, UID 1000/userns, held FD i reopen. Python VM 3.13.5 oraz `trackfd=False` rozdzielają mmap od FD; rzeczywiste ENODEV 19 przy `cache.files=off` oznacza wariant nieprzetestowany. Direct backing write jest kontrolą zakresu FUSE, nie izolacją branchy. R1 `kaD6aK` odmówiło przed remount na `/proc/self/setgroups` (errno 13, wtórny KeyError), zachowując `prepare/pending=actor-start`; jego dumpability nie zmierzono. R2 na opublikowanym `3dc2933f…` (exact 214 PASS) potwierdziło root→1000/caps 0, dumpability 0→1, mapowania i RLIMIT_CORE 0. Preflight/prepare kod 0, local FAILED: remount 0, umount EBUSY 16, oba aliasy nadal zapisują; journal `prepare/pending=reopen`, wszystkie dzieci zakończone, osiem plików zgodne SHA/stat, dodatkowe dyski puste. Przyczyna busy w diagnozie; global niewykonane. Następnie osobny pomiar globalnego RO bez resetu starego journala. A0/E2-09 i izolacja direct branch pozostają otwarte, nie gotowa bramka produktu ani mover. Stare VM i pending nietknięte, bez retry/reset/cleanup.
