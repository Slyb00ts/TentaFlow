...
- Adaptery Sync/Scrub, router i konsument UI są zintegrowane. Exact testy core/helper, nowy WASM i NAS Playwright zaliczono; rzeczywisty odbiór całego Sync/Scrub z historią po restarcie nadal pozostaje otwarty. Pierwszy live Sync zachował failed/needs_attention: narzędzie zakończyło niezmieniony Sync jako `Nothing to do` bez `Everything OK`, czego pierwotna fixture parsera nie odzwierciedlała. Regresja używa dosłownego logu i stdout; poprawka parsera nie czyści istniejącego pending ani nie przepisuje historii. Historyczny pomiar CLI nie zastępuje udanego odbioru API.
