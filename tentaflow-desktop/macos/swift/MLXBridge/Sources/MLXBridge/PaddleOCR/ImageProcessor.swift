...
public struct ProcessedImages: Sendable {
    public let pixelValues: MLXArray   // [1, H, W, 3] znormalizowany obraz
    public let width: Int
    public let height: Int
    public let gridH: Int              // H / patch
    public let gridW: Int              // W / patch
    public let numImageTokens: Int     // (gridH/merge) * (gridW/merge)

    public init(pixelValues: MLXArray, width: Int, height: Int, gridH: Int, gridW: Int, numImageTokens: Int) {
...
public struct PaddleOCRVLImageProcessor {
    // OPENAI_CLIP_MEAN / STD (referencja PaddleOCR-VL).
    public static let imageMean: (Float, Float, Float) = (0.48145466, 0.4578275, 0.40821073)
    public static let imageStd: (Float, Float, Float) = (0.26862954, 0.26130258, 0.27577711)
    public static let mergeSize: Int = 2
    ...
    public func process(_ image: CIImage) -> ProcessedImages {
        ...
        let (targetWidth, targetHeight) = smartResize(width: originalWidth, height: originalHeight)
        ...
        let m = Self.mergeSize
        let numImageTokens = (gridH / m) * (gridW / m)
    ...
    /// Resize aspect-preserving do wielokrotnosci patch*merge (28), z capem
    /// maxPixels. Gwarantuje gridH/gridW parzyste (warunek spatial-merge 2x2).
    private func smartResize(width: Int, height: Int) -> (Int, Int) {
        let factor = patchSize * Self.mergeSize  // 28
        ...
        let newWidth = max(factor, Int((Double(width) * scale / Double(factor)).rounded()) * factor)
        let newHeight = max(factor, Int((Double(height) * scale / Double(factor)).rounded()) * factor)
