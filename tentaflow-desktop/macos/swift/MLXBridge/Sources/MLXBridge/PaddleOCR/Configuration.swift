...
public struct PaddleOCRVLConfig: Codable, Sendable {
    ...
    public static func load(from directory: URL) throws -> PaddleOCRVLConfig {
        ...
        // Pola LLM sa na TOP-LEVEL config.json (brak `text_config`) — dekodujemy
        // PaddleOCRVLTextConfig z calego configu.
        ...
        if let textDict = raw["text_config"] as? [String: Any] {
            ...
            textConfig = (try? decoder.decode(PaddleOCRVLTextConfig.self, from: data)) ?? PaddleOCRVLTextConfig()
        }

        // Placeholder obrazu w sekwencji to `image_token_id` (brak `vision_token_id`).
        let imageTokenIndex = (raw["image_token_id"] as? Int) ?? 100295
        let visionStartTokenId = (raw["vision_start_token_id"] as? Int) ?? 101305
        let visionEndTokenId = (raw["vision_end_token_id"] as? Int) ?? 101306
        let visionTokenId = (raw["image_token_id"] as? Int) ?? 100295
